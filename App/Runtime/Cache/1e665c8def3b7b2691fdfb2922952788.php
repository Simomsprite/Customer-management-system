    ...
    <div class="row">
	    <div class="col-lg-12">
	    	<div class="ibox-content all-inline title-strip">
				...
				<span><h1><?php echo ($contacts['name']); ?></h1></span>
				<span class="fa fa-qrcode fa-2x" id="qrcode"></span>
				<?php if($contacts['role']): ?><span class="badge badge-primary m-b-sm m-l-sm" style="padding:4px 12px;"><big><?php echo ($contacts['role']); ?></big></span><?php endif; ?>
				<a href="<?php echo U('contacts/edit','id='.$contacts['contacts_id']);?>" class="btn btn-outline btn-default pull-right" style="margin-right: 15px;"><i class="fa fa-pencil"></i>&nbsp;&nbsp;编辑</a> 
	    	</div>
			    		    ...
			    		    <div class="panel-body">
			    		        <div class="ibox">
									<div class="form-horizontal view-group">
										<?php $tag = 0; ?>
										<?php if(is_array($field_list)): $key = 0; $__LIST__ = $field_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key; if($vo['form_type'] == 'textarea'): if($tag%2 != 0): ?></div><?php endif; ?>
											<div class="form-group">
												<label class="col-sm-2 control-label"><?php echo ($vo["name"]); ?></label>
											    <div class="col-sm-10">
											        <p class="form-p color-a-edit">
											            <span style="color:#<?php echo ($vo['color']); ?>"><?php echo ($contacts[$vo['field']]); ?></span>
											        </p>
											    </div>
											</div>
											<?php $tag = 0; ?>
										    ...
										    <div class="col-sm-4">
										        <p class="form-p">
										            <span style="color:#<?php echo ($vo['color']); ?>">
														<?php if($vo['form_type'] == 'datetime'): if($contacts[$vo['field']] != '0'): echo newTimeDate($contacts[$vo['field']]); endif; ?>
														<?php elseif($vo['form_type'] == 'address'): ?>
															<?php echo ($contacts[$vo['field']]); ?>
															<a href="javascript:void(0);" class="getMap" rel="<?php echo ($contacts[$vo['field']]); ?>">
																<span class="fa fa-map-marker" style="font-size:16px;"></span>
															</a>
														<?php elseif($vo['field'] == 'customer_id'): ?>
															<a href="<?php echo U('customer/view','&id='.$contacts['customer_id']);?>"><?php echo ($contacts['customer_name']); ?></a>
														<?php else: ?>
															<?php echo ($contacts[$vo['field']]); endif; ?>
										            </span>
										        </p>
										    </div>
											<?php if($tag%2 != 0 || $key == count($field_list)): ?></div><?php endif; ?>
			...
			<div class="pull-right" style="width:29.7%;">
				...
				<div class="ibox-content" style="border-top:none;">
					<table class="table ">
						...
						<tbody>
							<?php if(is_array($business_list)): $i = 0; $__LIST__ = $business_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
									<td ><a href="<?php echo U('business/view','id='.$vo['business_id']);?>"><?php echo ($vo['code']); ?></a></td>
									...
									<td class="dropdown">
										...
										<ul class="dropdown-menu">
										    <li>
										    	<a rel="<?php echo U('contacts/relToBusiness',array('business_id'=>$vo['business_id'],'contacts_id'=>$vo['contacts_id'],'act_n'=>2));?>" class="not_rel_business" href="javascript:void(0);" >解除关联</a>
										    </li>
										</ul>
									</td>
...
<div style="display:none;" id="dialog-log" title="<?php echo L('DIALOG_ADD_LOG');?>">
	<div class="spiner-example">
        <div class="sk-spinner sk-spinner-three-bounce">
            <div class="sk-bounce1"></div>
            <div class="sk-bounce2"></div>
...
<div style="display:none;" id="dialog-log-edit" title="<?php echo L('DIALOG_EDIT_LOG');?>">
	<div class="spiner-example">
        <div class="sk-spinner sk-spinner-three-bounce">
            <div class="sk-bounce1"></div>
            <div class="sk-bounce2"></div>
...
function url_jump(maodian){
    var leads_id = "<?php echo ($contacts['contacts_id']); ?>";
    var url = "<?php echo U('contacts/view','id=');?>"+leads_id+maodian;
...
$("#qrcode").popover({
    ...
    title : '',
    html: 'true',
    content : '<div id="" rel=""><p style="text-align:center;background:#fff;"><small>扫一扫下面的二维码就可以把联系人<br/>保存到手机里啦</small></p><img style="margin:0 auto;width:200px;height:200px;" src="<?php echo U('contacts/qrcode', 'contacts_id='.$contacts['contacts_id']);?>" /></div>',
    animation: false
...
$(document).on('click','#rel_business',function () {
	...
	$('#dialog-business').load("<?php echo U('customer/clistDialog','customer_id='.$contacts['customer_id'].'&contacts_id='.$contacts['contacts_id']);?>");
});
$('#dialog-business').dialog({
	...
	buttons: {
		"确定": function () {
			...
			var contacts_id = "<?php echo ($contacts['contacts_id']); ?>";
			var to_url = "<?php echo U('contacts/relToBusiness');?>&act_n=1&contacts_id="+contacts_id+"&business_id="+business_id;
...
$(function(){
	$(".add_file").click(function(){
		...
		$('#dialog-file').load('<?php echo U("file/add", "r=RContactsFile&module=contacts&id=".$contacts["contacts_id"]);?>');
	});
	$(".add_log").click(function(){
		$('#dialog-log').dialog('open');
		$('#dialog-log').load('<?php echo U("log/add", "r=RContactsLog&module=contacts&id=".$contacts["contacts_id"]);?>');
	});
