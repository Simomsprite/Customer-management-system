				...
				<div class="title-bar" style="position: relative;z-index: 99;">
					<div class="row  clearfix" id="title-hide" style="display:none;">
						<ul class="breadcrum pull-left">
							...
							<?php if(checkPerByAction('contacts','excel_export')): ?><li><a href="javascript:void(0)" class="export_excel"><i class="fa fa-download"></i>&nbsp;导出</a></li><?php endif; ?>
					...
					<div class="row " id="title-show">
						...
						<div class="btn-group pull-right" style="margin-right: 30px;">
	                        ...
	                        <ul class="dropdown-menu">
	                            <?php if(checkPerByAction('contacts','excel_import')): ?><li><a id="import_excel" class="link" href="javascript:void(0);"><i class="fa fa-upload"></i>&nbsp;导入</a></li><?php endif; ?>
								<?php if(checkPerByAction('contacts','excel_export')): ?><li><a href="javascript:void(0);" class="link export_excel"><i class="fa fa-download"></i>&nbsp;导出</a></li><?php endif; ?>
	                        </ul>
	                    </div>
						<!-- END 导入、导出 -->
						<form class="form-inline" id="contacts_search" method="get">
							<ul class="breadcrum pull-right" style="padding-right: 15px;">
								<li>
									搜索：
									<div class="input-group">
										...
										<input id="short_search" type="text" style="width:160px;" p
											   laceholder="<?php echo L('PLEASE_INPUT_A_CONTACT_NAME');?>" onkeydown='if(event.keyCode==13) {$("#short_search_btn").trigger("click");return false;}' class="form-control input-sm" name="search" <?php if($_GET['field'] == 'name'): ?>value="<?php echo ($_GET['search']); ?>"<?php endif; ?>/>
				...
				<div class="ibox-content clearfix" id="table_container" style="z-index: 1;">
					<form id="form1" action="" method="post" style="position:relative;">
						...
						<table class="table table-hover table-striped table_thead_fixed" id="tab_Test3">
							<?php if($contactsList == null): ?><div style="background-color:#fff;"><div style="text-align:center;color:#E4E4E4;font-size:22px;font-weight:700;padding:15px 0px;">
...
<div style="display:none" id="dialog-export" title="导出客户联系人">
		...
		<div id="search_add" style="width:100%;float:left;padding-bottom:10px;">
			...
			<?php if(is_array($fields_search)): $key1 = 0; $__LIST__ = $fields_search;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key1 % 2 );++$key1;?><div id="con_search_<?php echo ($key1); ?>" style="float:left;width:100%;margin:10px 10px 0 10px;">
					...
					<ul class="nav pull-left" style="margin:0px 0 0 5px;width:88%;">
						...
						<?php elseif($vo["form_type"] == 'customer'): ?>
							<!-- “所属客户”搜索无中间条件 -->
						<?php else: ?>
							<select id="condition" style="width:auto" class="form-control input-sm" name="<?php echo ($vo['field']); ?>[condition]">
...
$("#dialog-import").dialog({
	...
	buttons: {
		"确定": function () {
			location.href = "<?php echo U('contacts/index');?>";
		}
	}
});
...
var url = "<?php echo U('contacts/getcurrentstatus');?>";
var limit_size = 1000;
...
$(function(){
	...
	$("#import_excel").click(function(){
		...
		$('#dialog-import').load('<?php echo U("contacts/excel_import");?>');
	});

	// 导出
	$('.export_excel').on('click', function(){
		if (<?php echo count($contactsList);?> == 0) {
			swal('数据为空', '请变更查询条件后再执行导出。', 'warning');
			return false;
		}
		var id_array = new Array();
