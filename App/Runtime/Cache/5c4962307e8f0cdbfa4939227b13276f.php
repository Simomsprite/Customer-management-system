...
<div class="wrapper wrapper-content animated fadeIn col-md-6">
	<form class="form-horizontal" id="form_leads" role="form" action="<?php echo U('leads/edit');?>" method="post">
	<input type="hidden" name="leads_id" id="leads_id" value="<?php echo ($leads["leads_id"]); ?>"/>
	<input type="hidden" name="jump_url" value="<?php echo ($jump_url); ?>"/>
				...
				<div class="full-height-scroll">
					<div class="row" >
						...
						<div class="col-md-11 add_body_form">
							<?php if(is_array($field_list['main'])): $key = 0; $__LIST__ = $field_list['main'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><div class="form-group">
						...
						<div class="col-md-11 add_body_form">
							<?php if(is_array($field_list['data'])): $key = 0; $__LIST__ = $field_list['data'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><div class="form-group">
									<label class="col-md-4 control-label"><?php echo ($vo["name"]); ?>：</label>
									<?php if($vo['form_type'] == 'address' || $vo['form_type'] == 'box'): ?><div class="col-md-7">
											<?php echo ($vo["html"]); ?>
										</div>
...
<div id="dialog-role-list" class="" style="display:none" title="<?php echo L('CHOOSE_THE_LEADS_OWNER');?>">loading...</div>
<div class="" style="display:none" id="dialog-transform" title="<?php echo L('CONVERT_LEADS');?>">
...
<div class="" style="display:none" id="transform-role-list" title="<?php echo L('CHOOSE_THE_LEADS_OWNER');?>">loading...</div>
<div class="" style="display:none" id="dialog-validate" title="<?php echo L('VALIDATE_COMPANY_RESULT');?>">
...
function checkinfo(param){
	...
	if(field_value){
		$.post('<?php echo U("leads/checkinfo");?>',
			{
...
$(function(){
	...
	$('#name').blur(
		function(){
			...
			if(name!=''){
				$.post('<?php echo U("leads/check","leads_id=");?>'+leads_id,
					{
						name:name
					},
					function(data){
