				...
				<div class="title-bar" style="position: relative;z-index: 99;">
					<div class="row  clearfix" id="title-hide" style="display:none;">
						<ul class="breadcrum pull-left">
							...
							<li class="single_btn"><a href="javascript:void(0)" id="edit_purchase"><i class="fa fa-pencil"></i>&nbsp;编辑</a></li>
					...
					<div class="row" id="title-show">
						<ul class="nav pull-left" style="margin:2px 0 0 15px;">
							<a href="<?php echo U('purchase/add');?>" class="btn btn-primary btn-sm pull-left" style="margin-right:8px"><i class="fa fa-plus-circle"></i>&nbsp; 添加采购单</a> 
						</ul>
						<form class="form-inline" id="purchase_search" action="" method="get">
							...
							<ul class="breadcrum pull-right" style="margin-bottom: 0px">
								...
								<li>
									...
									<div class="input-group">
										...
										<input id="short_search" type="text" style="width:210px;" placeholder="请输入采购单编号/采购名称/供应商名称/" onkeydown='if(event.keyCode==13) {$("#short_search_btn").trigger("click");return false;}' class="form-control input-sm" name="search" <?php if($_GET['field'] == 'name'): ?>value="<?php echo ($_GET['search']); ?>"<?php endif; ?>/>
	...
	<img src="__PUBLIC__/img/exclamation_mark.png" style="margin-top:-3px;">&nbsp;&nbsp;暂无数据
								...
								<?php else: ?>
									...
									<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="controls_tr">
											...
											<td>
												<a href="<?php echo U('supplier/view','id='.$vo['supplier_id']);?>"><?php echo ($vo['supplier_name']); ?></a>
											</td>
											...
											<td>
												<span class="fa fa-circle examine" style="color:<?php echo ($vo['exam_status']['color']); ?>"></span><?php echo ($vo['exam_status']['name']); ?>
											</td>
				...
				<ul class="nav pull-left" style="margin:0px 0 0 23px;width:650px">
					<li class="pull-left">
						<select id="field_1" style="width:auto" class="form-control input-sm field_name new-select" onchange="changeCondition(1)" >
							...
							<option class="word" value="code">采购单编号</option>
					...
					<ul class="nav pull-left" style="margin:0px 0 0 5px;width:620px;">
						<li class="pull-left">
							<select id="field_<?php echo ($key1); ?>" style="width:auto" class="form-control input-sm field_name new-select" onchange="changeCondition(<?php echo ($key1); ?>)" >
								<option class=""  value="name">--请选择条件--</option>
								<option class="word" <?php if($vo['field'] == 'code'): ?>selected="selected"<?php endif; ?> value="code">采购单编号</option>
...
$("#edit_purchase").click(function(){
	window.location.href="<?php echo U('purchase/edit', 'id=');?>"+$(this).attr('rel');
});
...
$('#delete').click(function(){
	...
	function(isConfirm){
        if (isConfirm) {
        	$.ajax({
	            type:'post',
	            url: "<?php echo U('purchase/delete');?>",
...
$('.excelExport').on('click', function () {
	if ($(this).attr('type') == 'id') {
		...
		$('#purchase_search [name="ids"]').val(ids.join(','));
	}
	$('#purchase_search [name="act"]').val('export');
	$('#purchase_search').submit();
	$('#purchase_search [name="act"]').val('');
});
</script>
