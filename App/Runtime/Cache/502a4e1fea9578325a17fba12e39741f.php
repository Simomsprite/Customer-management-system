...
<div class="wrap-box">
  <header class="abe-header am-header-default"> <a href="javascript:goIndex();" class="return iconfont">&#xe612;</a>
    <h1 class="abe-header-title">新建客户</h1>
  </header>
    <form action="<?php echo U('customerm/add');?>" method="post">
  ...
  <ul class="cust-form">
    <?php if(is_array($field_list['main'])): $key = 0; $__LIST__ = $field_list['main'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><li class="titbox">
        ...
        <?php if($vo['field'] == 'customer_owner_id'): ?><div class="col-md-6">
          ...
          <?php elseif($vo['field'] == 'customer_code'): ?>
          <div class="col-md-6">
            <input class="txt ptxt100p" type="text" id="customer_code" name="customer_code" value="<?php echo ($customer_code); ?>" aria-required="true">
          </div>
          <div class="col-md-2"></div>
          ...
          <div class="col-md-6">
            <?php if($vo['field'] == 'grade'): ?><input type="hidden" id="star" name="grade" value="">
              <div class="all-star" style="font-size: 18px;padding-top:15px;color:#D0D0D0;">
  ...
  <div class="sub-box">
    <input type="button" value="下一步" name="button" class="pbtn pbtn-sm pbtnw30p mrg_15" onclick="return myValidate1();">
      <input type="button" value="取消" onclick="goIndex()" name="button" class="pbtn  pbtnw30p pbtn-sm">
  </div>
     </div>
...
<div id="pagetwo" style="display: none">
        <ul class="cust-form">
            <?php if(is_array($contacts_field_list['main'])): $key = 0; $__LIST__ = $contacts_field_list['main'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key; if($vo['form_type'] != 'customer'): ?><li class="titbox">
                        <span class="tit"><?php echo ($vo["name"]); ?>：<?php if($vo["is_null"] == '1' && $vo["is_validate"] == '1' && $vo["is_unique"] == '1'): ?><em class="abe-red">*</em><?php endif; ?></span>
        ...
        <div class="sub-box">
            <input type="button" name="button" value="上一步" class="pbtn  pbtnw30p pbtn-sm mrg_15" onclick="getnext('pagetwo')">
            <input type="submit" name="button" value="创建客户" class="pbtn pbtn-sm pbtnw30p " onclick="return myValidate()">
        </div>
</div>
    </form>
</div>
...
function myValidate1() {
    ...
    if (name == "") {
        ...
        showtab1(2, "客户编号不能为空");
        return false;
    }else if (codeReg.test(customer_code) == false) {
...
function myValidate(){
    ...
    var codeReg=/^[0-9a-zA-Z_]{1,14}$/;
    if (codeReg.test(customer_code)==false){
        showtab1(2,"客户编号格式不正确");
        return false;
    }
    ...
    /*var qq=$("con_contacts\[qq_no\]").val();
    alert(qq);
    var qqReg=/^[1-9]\d{4,9}$/;
    console.log(qqReg.test(qq));
    if (qqReg.test(qq)==false){
        showtab1(2,"qq格式不正确");
        return false;
    }*/

    /*var mobile=$("#con_contacts[telephone]").val();*/
    var customerMobile=$("#crm_vwlnfx").val();
   /* let phoneReg = /^$|^[1-9][0-9]{4,14}$/;
    if (phoneReg.test(mobile)==false) {
        showtab1(2,"手机格式不正确");
        return false;
    }*/
    let customerMobileReg = /^$|^1(3|4|5|6|7|8|9)\d{9}$/;
    if(customerMobileReg.test(customerMobile)==false) {
