            	...
            	<div class="title-bar" style="position: relative;z-index: 99;">
					...
					<div class="row " id="title-show">
						<ul class="nav pull-left" style="margin:2px 0 0 15px;">
							<a href="<?php echo U('business/add');?>" class="btn btn-primary btn-sm pull-left" style="margin-right:8px"><i class="fa fa-plus-circle"></i>&nbsp; 新建商机</a>
						</ul>
						<form class="form-inline" id="business_search" action="" method="get">
							...
							<ul class="breadcrum pull-right" style="margin-bottom: 0px; padding-right: 15px;">
								<li>
									<div class="input-group" style="margin-right: 10px;margin-bottom: 5px;">
										<select class="form-control" onchange="window.open(this.options[this.selectedIndex].value,target='_self')" >
											<option value="<?php echo U('business/index','by=me&'.$by_parameter);?>" <?php if($_GET['by'] == 'me' || $_GET['by'] == ''): ?>selected="selected"<?php endif; ?>>我的商机</option>
											<option value="<?php echo U('business/index','by=sub&'.$by_parameter);?>" <?php if($_GET['by'] == 'sub'): ?>selected="selected"<?php endif; ?>>下属商机</option>
											<option value="<?php echo U('business/index','by=all&'.$by_parameter);?>" <?php if($_GET['by'] == 'all'): ?>selected="selected"<?php endif; ?>>全部商机</option>
										</select>
									</div>
								</li>
								<li>
									...
									<div class="input-group">
										...
										<input id="short_search" type="text" style="width:200px;" placeholder="商机名称" onkeydown='if(event.keyCode==13) {$("#short_search_btn").trigger("click");return false;}' class="form-control input-sm" name="name[value]" <?php if($_GET['name']['condition'] == 'contains'): ?>value="<?php echo ($_GET['name']['value']); ?>"<?php endif; ?> />
										...
										<select class="form-control" name="status_type_id" id="status_type_id" style="margin:0px 10px;width:150px;">
											...
											<?php if(is_array($status_type_list)): $i = 0; $__LIST__ = $status_type_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['id']); ?>"><?php echo ($vo['name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
										</select>
										<select class="form-control input-sm" style="width:auto" name="status_condition">
	...
	<img src="__PUBLIC__/img/exclamation_mark.png" style="margin-top:-3px;">&nbsp;&nbsp;暂无数据
								...
								<?php else: ?>	
									...
									<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="controls_tr">
											...
											<td>
												<?php if($vo['set_top'] == 1): ?><a style="border-left: 5px solid #ffb07b;padding-left: 5px;" href="<?php echo U('business/view','id='.$vo['business_id']);?>" ><?php echo ($vo['name']); ?></a>
												<?php else: ?>
													<a style="padding-left: 5px;" href="<?php echo U('business/view','id='.$vo['business_id']);?>" ><?php echo ($vo['name']); ?></a><?php endif; ?>
											</td>
											<?php if(is_array($field_array)): $i = 0; $__LIST__ = $field_array;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; if($v['field'] == 'customer_id'): ?><td>
														<a href="<?php echo U('customer/view','id='.$vo['customer_id']);?>">
												...
												<?php else: ?>
													<td>
														<span style="color:#<?php echo ($v['color']); ?>">
															<?php if($v['form_type'] == 'datetime'): if($vo[$v['field']]): echo ($vo[$v['field']]); endif; ?>
															<?php else: ?>
						...
						<div id="tfoot_div" class="clearfix">
							<div class="clearfix" id="tfoot_page">
								<?php if($fields_search || $_GET['field']): ?><span class="pull-left" style="margin-left:25px;margin-top:10px;">本次搜索结果<span style="color:#F8AC59"> <?php echo ($count); ?> </span>条数据<a href="<?php echo U('business/index');?>" class="btn" style="background:#fff;border:1px solid #ccc;margin-left:10px;color:#999;" id="clearnumber">清除搜索条件</a></span><?php endif; ?>
...
// 商机状态condition条件搜索后选中
if ('<?php echo ($_GET['status_id']); ?>' == '') {
	$('select[name="status_condition"]').val('eq');
} else {
	$('select[name="status_condition"]').val("<?php echo ($_GET['status_condition']); ?>");
}
// 选择商机状态condition条件
$('select[name="status_condition"]').change(function(){
	if ($('#status_id').val() != '') {
		status_submit();
	}
});
...
//状态分组
var status_type_id = "<?php echo (($_GET['status_type_id'])?($_GET['status_type_id']):0); ?>";
$('#status_type_id').val(status_type_id); 
$('#status_type_id').change(function(){
	var type_id = $(this).val();
	var temp = '<option value="">--全部商机进度--</option>';
	if(type_id){
		$.ajax({
			'type':'get',
...
function status_submit() {
	...
	_get['m'] = 'business';
	_get['a'] = 'index';
	_get['status_type_id'] = $('#status_type_id').val();
	_get['status_id'] = $('#status_id').val();
	_get['status_condition'] = $('select[name="status_condition"]').val();
	var param = $.param(_get);
	if (param == '') {
		param += 'm=business&a=index&status_type_id='+ $('#status_type_id').val() +'&status_id=' + $('#status_id').val() + '&status_condition=' + $('select[name="status_condition"]').val();
	}
...
$(function(){
	...
	$('.excelExport').on('click', function () {
		if ($(this).attr('type') == 'id') {
			...
			$('#business_search [name="ids"]').val(ids.join(','));
		}
		$('#business_search [name="act"]').val('export');
		$('#business_search').submit();
		$('#business_search [name="act"]').val('');
	});
});
</script>
<!-- 地址插件 -->
