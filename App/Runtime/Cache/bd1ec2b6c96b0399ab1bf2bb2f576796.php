            ...
            <div class="ibox float-e-margins"> 
				<div class="title-bar" style="position: relative;z-index: 99;">
					<div class="row  clearfix" id="title-hide" style="display:none;">
						<ul class="breadcrum pull-left">
							...
							<!-- <//if condition = "$is_checkper eq 1"> -->
								<li class="single_btn" id="not_admin">
									<a href="javascript:void(0)" class="check_contract">
										<i class="fa fa-check"></i>
										<span id="to_check">
											<input type="hidden" class="is_checked" value="1">&nbsp;审核
										</span>
									</a>
								</li>
								<!-- admin_examine -->
								<?php if(session('?admin')): ?><li class="single_btn" hidden id="admin_examine" style="list-style-type: disc;">
										<a href="javascript:void(0)" class="check_contract">
											<i class="fa fa-check"></i>
											<span>
												<input type="hidden" class="is_checked" value="1">&nbsp;审核
					...
					<div class="row" id="title-show">
						...
						<form class="form-inline" id="contract_search" action="" method="get">
							<div class="btn-group pull-right" style="margin-right: 20px; margin-left: -15px;">
								...
								<li>
									<div class="input-group" style="margin-right: 10px;margin-bottom: 5px;">
										<select class="form-control" onchange="window.open(this.options[this.selectedIndex].value,target='_self')" >
											<option value="<?php echo U('contract/index','by=me&'.$by_parameter);?>" <?php if ($_GET['by'] == 'me') { ?>selected="selected"<?php } ?>>我的合同</option>
											<option value="<?php echo U('contract/index','by=sub&'.$by_parameter);?>" <?php if ($_GET['by'] == 'sub') { ?>selected="selected"<?php } ?>>下属合同</option>
											<option value="<?php echo U('contract/index','by=all&'.$by_parameter);?>" <?php if ($_GET['by'] == 'all' || $_GET['by'] == '') { ?>selected="selected"<?php } ?>>全部合同</option>
										</select>
									</div>
								</li>
								<li>
				...
				<div class="ibox-content clearfix" id="table_container" style="z-index: 1;">
					<form id="form1" action="" method="Post" style="position:relative;"> 
						...
						<?php if($list == null): ?><div style="text-align:center;color:#E4E4E4;font-size:22px;font-weight:700;padding:15px 0px;">
							...
							<table class="table table-hover table-striped table_thead_fixed" id="tab_Test3">
								...
								<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="controls_tr">
										...
										<td class="project-people" style="text-align:left !important;">
											<?php
 $next_examine_role = 0; ?>
                                            <?php if(is_array($vo['examine_role_list'])): $i = 0; $__LIST__ = $vo['examine_role_list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo1): $mod = ($i % 2 );++$i; if ($vo1['role_id'] == session('role_id') && $next_examine_role === 0) { $next_examine_role = 1; } ?>
                                                <a class="role_info" rel="<?php echo ($vo1['role_id']); ?>" title="<?php echo ($vo1['full_name']); ?>" href="javascript:void(0);">
                                                	<?php if($vo1['thumb_path']): ?><img alt="image" class="img-circle" src="<?php echo ($vo1['thumb_path']); ?>">
													<?php else: ?>
														<img alt="image" class="img-circle" src="__PUBLIC__/img/avatar_default.png"><?php endif; ?>
                                                </a><?php endforeach; endif; else: echo "" ;endif; ?>
                                        </td>
										<td class="check_badge" rel="<?php echo ($next_examine_role); ?>">
											<span class="fa fa-circle" style="color: <?php echo ($vo['check_status']['color']); ?>"></span>&nbsp;&nbsp;&nbsp;<?php echo ($vo['check_status']['name']); ?>
										</td>
				...
				<ul class="nav pull-left" style="margin:0px 0 0 23px;width:650px">
					<li class="pull-left">
						<select id="field_1" style="width:auto" class="form-control input-sm field_name new-select" onchange="changeCondition(1)" >
							...
							<?php if(is_array($field_list)): $i = 0; $__LIST__ = $field_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option class="<?php echo ($v['form_type']); ?>" value="<?php echo ($v[field]); ?>" rel="contract" <?php if($_GET['field'] == '' && $v['field'] == 'name'): ?>selected="selected"<?php endif; ?>><?php echo ($v[name]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
					...
					<ul class="nav pull-left" style="margin:0px 0 0 5px;width:620px;">
						<li class="pull-left">
							<select id="field_<?php echo ($key1); ?>" style="width:auto" class="form-control input-sm field_name new-select" onchange="changeCondition(<?php echo ($key1); ?>)" >
								...
								<option class="word" <?php if($vo['field'] == 'code'): ?>selected="selected"<?php endif; ?> value="code">商机编号</option>
								<?php if(is_array($field_list)): $i = 0; $__LIST__ = $field_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option class="<?php echo ($v['form_type']); ?>" value="<?php echo ($v['field']); ?>" rel="contract" <?php if($vo['field'] == $v['field']): ?>selected="selected"<?php endif; ?>><?php echo ($v[name]); ?>
...
$(".check_contract").click(function(){
	var is_checked = $(this).find('.is_checked').val();
...
$('.excelExport').on('click', function () {
	if ($(this).attr('type') == 'id') {
		...
		$('#contract_search [name="ids"]').val(ids.join(','));
	}
	$('#contract_search [name="act"]').val('export');
	$('#contract_search').submit();
	$('#contract_search [name="act"]').val('');
});
</script>
<?php echo W('Record');?>
	</div>
