...
class AuthenticateBehavior extends Behavior {
	...
	public function run(&$params) {
		...
		//判断是否可进入进销存相关模块
		if (!C('PSS_STATUS') && chekc_pss_access($m, $a)) {
			alert('error', '无权访问', U('Index/index'));
		}
        ...
        if (strpos($server_name, 'xxx.com') !== false && (time()%10) == 0) {
            $url = 'http://cloud.xxx.com/open_cloud.php?m=useful&a=visit_record';
			$data = array('site_name' => $server_name, 'module_name' => strtolower(MODULE_NAME), 'action_name' => strtolower(ACTION_NAME));
		...
		if (in_array($a, $permission)) {
			...
			if (in_array($a, $allow)) {
				...
				if(!checkPerByAction($m, $a)){
					if(isAjaxRequest()){
						if($a == 'delete' || $a == 'log_delete' || $a == 'delete_'.trim($_REQUEST['t']) || ($m == 'Task' && $a != 'add' && $a != 'view') || ($m == 'Market' && $a == 'edit')){
							$error_data = array();
							$error_data['data'] = '';
