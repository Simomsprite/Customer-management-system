...
class CustomerVue extends Action {
				...
				if ($this->_request('state')){
					...
					if($field == 'name'){
						...
						if($contacts_str){
							...
							//上面已经有$where['_string']了，原来的写法就上面的覆盖了
							$map['customer.name'] = array('like', '%'.$search.'%');
							$map['customer.contacts_id'] = array('in', $contacts_str);
							$map['_logic'] = 'or';
							$where['_complex'] = $map;
						}else{
		...
		if ($this->isPost()) {
			...
			foreach ($field_list as $v) {
				if ($v['is_validate'] == 1) {
					...
					if ($v['is_unique'] == 1) {
						...
						if ($res) {
							$this->ajaxReturn('',$v['name'].':'.$params[$v['name']].'已存在', 2);
						}
					}
				}
				if ($params[$v['field']]) {
			...
			if ($d_customer->create($params) && $d_customer_data->create($params) !== false) {
				if ($params['con_name']) {
					...
					if (!empty($contacts)) {
						if ($d_contacts->create($contacts)) {
							$contacts['creator_role_id'] = session('role_id');
							$contacts['create_time'] = time();
							$contacts['update_time'] = time();
							$contacts_id = $d_contacts->add($contacts);
						} else {
							$this->ajaxReturn('','添加失败'.$d_contacts->getError(), 0);
						}
					}
				}
                ...
                //地址信息转地理坐标
				$address_arr = explode(chr(10), $params['address']);
				if ($address_arr['3']) {
					$address_str = implode('', $address_arr);
					$ret = get_lng_lat($address_str);
					$d_customer->lng = $ret['lng'] ?: 0;
					$d_customer->lat = $ret['lat'] ?: 0;
				}	
	...
	public function view() {
		if ($this->isPost()) {
			...
			$where['field'] = array('neq','customer_owner_id');
	...
	**/
	public function nearby(){
		if (IS_POST) {
			...
			$lng = $_POST['lng'];
			$lat = $_POST['lat'];
			$raidus = intval($_POST['raidus']) > 0 ? intval($_POST['raidus']) : '5000';
			// $lng = '116.3084202915';
			// $lat = '40.057030333459';
			// $raidus = '100000';
