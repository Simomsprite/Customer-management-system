...
class LeadsVue extends Action {
	...
	public function index(){
		if ($this->isPost()) {
			...
			switch ($by) {
				...
				case 'sub' : $where['leads.owner_role_id'] = array('in',implode(',', $below_ids)); break;
				...
				case 'me' : $where['leads.owner_role_id'] = session('role_id'); break;
				default :
			...
			if ($this->_permissionRes && !isset($where['leads.owner_role_id']) && $by != 'public') {
				if($by != 'deleted') {
					$where['leads.owner_role_id'] = array('in', $this->_permissionRes);
				} else {
					$where['leads.owner_role_id'] = array('in', '0,'.implode(',', $this->_permissionRes));
				}
			}
					...
					switch ($v['form_type']) {
						...
						case 'box':
							...
							if($field_type['type'] == 'checkbox'){
								$a = array_filter(json_decode($params[$v['field']]));
	...
	public function dynamic() {
		if ($this->isPost()) {
			...
			if (!$leads_info) {   
				$this->ajaxReturn(array(MODULE_NAME, ACTION_NAME),'数据不存在，或已删除',0);
			}
			...
			if ($leads_info['have_time'] >= $outdate) {
				...
				// 客户池
				$leads_info['owner'] = array('full_name' => '客户池', 'role_id' => 0);
			}
