...
class UserVue extends Action {
	...
	public function permission(){
		...
		$permission = array();
		$model = '';
		$existModel = array('customer','business','knowledge','contacts','product','leads','contract','announcement','examine','event','sign','task','invoice');
		foreach($row as $v){
			$tmp = explode('/',$v['url']);
		...
		//附近客户权限
		$nearby_per = $m_permission->where(array('position_id'=>session('position_id'),'url'=>'customer/nearby'))->find();
		if ($nearby_per) {
			$permission[] = 'nearby';
		}
		return $permission;
			...
			if ($user['password'] == md5(trim($_POST['password']) . $user['salt'])){
				if (0 == $user['status']) {
					...
					if (!is_array($role) || empty($role)) {
						...
						$data['img'] = headPathHandle($user['thumb_path']);

						$data['session_id'] = session_id();

						//生成token
						//$data['token'] = md5(md5($data['session_id']).time());
						$data['token'] = md5(md5($role['role_id']));
	...
	public function view(){
		if ($this->isPost()) {
			...
			$role_info['thumb_path'] = headPathHandle($role_info['thumb_path']);
			$role_info['img'] = headPathHandle($role_info['img']);
			$data['data'] = $role_info;
			$data['status'] = 1;
			$data['info'] = 'success';
			$this->ajaxReturn($data,'JSON');
	...
	public function edit(){
		if ($this->isPost()) {
			...
			if ($m_user->create()) {
				...
				$file = array();
				// 头像处理
				if ($img_id = (int) $_POST['img']) {
					$file = M('File')->where(array('file_id' => $img_id))->find();
					$m_user->img = $file['file_path'];
					$m_user->thumb_path = $file['file_path_thumb'];
				}
				...
				if ($m_user->where(array('role_id'=>$role_id))->save() || $is_update) {
					...
					if (!empty($file)) {
						...
						if ($oldImg = $user['img']) {
							if (file_exists($oldImg)) {
								@unlink($oldImg);
							} else if (strpos($oldImg, '/Uploads') === false) {
								$d_file->ossDelete(array($oldImg));
							}
						}
						M('File')->where(array('file_path_thumb' => $user['thumb_path']))->delete();
					}
					...
					$data_img = $file['file_path'] ? headPathHandle($file['file_path']) : headPathHandle($user['thumb_path']);
	...
	public function checkuser(){
		if ($this->isPost()) {
			...
			foreach ($role_list as $key => $val) {
				$role_list[$key]['thumb_path'] = headPathHandle($role_list[$key]['thumb_path']);
			}
