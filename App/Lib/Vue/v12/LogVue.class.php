...
class LogVue extends Action {
	...
	public function index() {
		if ($this->isPost()) {
			...
			$list = $d_log->distinct(true)->where($where)->field('log_id,role_id,subject,create_date,content')->order($order)->page($p.',10')->select();
			$count = $d_log->distinct(true)->where($where)->count();
			$page = ceil($count/10);
			$m_praise = M('Praise');
			foreach ($list as $k=>$v) {
				//过滤html代码
				...
				//评论数
				$list[$k]['comment_count'] = $m_log_talk->where(array('log_id'=>$v['log_id']))->count();
				//点赞数
				...
				foreach ($file_list as $key=>$val) {
					...
					$file_list[$key]['file_path'] = headPathHandle($file_list[$key]['file_path'], 1);
					$file_list[$key]['file_path_thumb'] = headPathHandle($file_list[$key]['file_path_thumb'], 1);
				}
				$list[$k]['file_list'] = $file_list ? $file_list : array();
				//作者
				$temp_val = $d_user->get_full_name(array($v['role_id']));
				$list[$k]['role_info'] = $temp_val[$v['role_id']];
			}
			//获取查询条件信息
			if($p == 1 && $_POST['search'] == ''){
				$fields_list = array(
	...
	public function mylog_view() {
		if ($this->isPost()) {
			...
			$log_info['img'] = headPathHandle($log_info['img']);
			if (!$log_info) {
				$this->ajaxReturn('','数据不存在或已删除！',0);
			}
			$role_id = session('role_id');
			...
			foreach ($comment_list as $k=>$v) {
				$temp_val = $d_user->get_full_name(array($v['send_role_id']));
				$user_info = $temp_val[$v['send_role_id']];
				$comment_list[$k]['user_name'] = $user_info['full_name'];
				$comment_list[$k]['img'] = headPathHandle($user_info['thumb_path']);
				...
				foreach ($comment_list_child as $key=>$val) {
					$temp_val = $d_user->get_full_name(array($val['send_role_id']));
					$creator_child_info = $temp_val[$val['send_role_id']];
					$comment_list_child[$key]['childimg'] = headPathHandle($creator_child_info['thumb_path']);
			...
			foreach ($praise_list as $k=>$v) {
				$temp_val = $d_user->get_full_name(array($v['role_id']));
				$praise_list[$k]['user_info'] = $temp_val[$v['role_id']];
			}
	...
	public function add(){
		if ($this->isPost()) {
			...
			$m_log->update_date = time();
			$m_log->status_id = (int) $_POST['status_id'] ?: (int) $_POST['type'];		// 历史遗留问题
			...
			if ($log_id = $m_log->add()) {
				if ($_POST['nextstep_time']) {
					//关联日程
					$event_id = dataEvent(trim($_POST['content']),$_POST['nextstep_time'],$module,$model_id);
				}
				$m_id = $module.'_id';
				$data['log_id'] = $log_id;
				$data[$m_id] = $model_id;
