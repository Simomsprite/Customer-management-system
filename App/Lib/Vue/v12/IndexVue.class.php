...
class IndexVue extends Action{
	...
	public function _initialize(){
		$action = array(
			'permission'=>array(),
			'allow'=>array('dynamic','index','fields','validate','loglist','filelist','work','scene', 'server_version')
		);
	...
	public function index() {
		if ($this->isPost()) {
			...
			foreach ($create_time as $k=>$v) {
				...
				$mylog_count[$v['name']] = $m_log->where(array('role_id'=>array('in',$briefing_role_ids),'category_id'=>array('neq',1),'create_date'=>$v['time']))->count();//工作日志
			}
			...
			if ($blows_id) {
				...
				//本周回款
				$sum_price_week = $m_receivingorder->where(array('receivables_id'=>array('in',$receivables_ids),'status'=>1,'pay_time'=>$create_time[0]['time']))->sum('money');
				//本月回款
				$sum_price_month = $m_receivingorder->where(array('receivables_id'=>array('in',$receivables_ids),'status'=>1,'pay_time'=>$create_time[1]['time']))->sum('money');
				//本年回款
				$sum_price_year = $m_receivingorder->where(array('receivables_id'=>array('in',$receivables_ids),'status'=>1,'pay_time'=>$create_time[2]['time']))->sum('money');
			...
			foreach ($all_role as $k=>$v) {
				...
				$temp_val = $d_user->get_full_name(array($v));
				$all_role_array[$k]['role_info'] = $temp_val[$v];
			}
	...
	public function dynamic(){
		if ($this->isPost()) {
			...
			foreach($log as $k=>$v){
				...
				$tmp['img'] = headPathHandle($role['thumb_path']);
				...
				if('sign' == $v['module_name']){
					...
					if ($v['module_name'] == 'contract') {
						$aname = M('Customer')->where('customer_id = %d',$contract_info['customer_id'])->getField('name');
					} else {
	...
	public function fields() {
		if ($this->isPost()) {
			...
			$array_m = array('business','leads','customer','contacts','product','contract','examine');
			//创建一个空对象
			...
			if ($m == 'examine') {
				if ($module_id) {
					$examine_info = M('Examine')->where(array('examine_id'=>$module_id))->field('type,examine_role_id')->find();
					if (!$examine_info['type']) {
			...
			if ($m == 'contract') {
				if ($module_id) {
					$contract_info = M('Contract')->where(array('contract_id'=>$module_id))->field('examine_role_id')->find();
				}
			}
			$where['model'] = $m;
				...
				if ($m && in_array($m, $array_m)) {
					...
					if ($m == 'contract') {
						...
						if ($contract_examine) {
							...
							if ($examine_role_ids) {
								$examine_role_list = $d_user->get_full_name(explode(',',$examine_role_ids));
								$examine_role_list = array_values($examine_role_list);
							}
						...
						//自定义字段
						$arr = array('customer_id','business_id','number','contract_name','owner_role_id','examine_role_id','price','due_time','start_date','end_date','description','product[]');
						$arr_contract = array('来源客户'=>'customer_id','来源商机'=>'business_id','合同编号'=>'number','合同名称'=>'contract_name','合同签约人'=>'owner_role_id','合同审批人'=>'examine_role_id','合同金额'=>'price','签约时间'=>'due_time','合同生效时间'=>'start_date','合同到期时间'=>'end_date','合同描述'=>'description','产品'=>'product[]');
						...
						foreach ($arr as $key=>$val) {
							...
							if ($val == 'number') {
								...
								$fields_list_contract[$key]['is_validate'] = '1';
								$fields_list_contract[$key]['is_null'] = '1';
							} elseif ($val == 'examine_role_id') {
								$fields_list_contract[$key]['setting'] = $examine_setting;
								$fields_list_contract[$key]['form_type'] = $examine_form_type;
								$fields_list_contract[$key]['is_validate'] = '1';
								$fields_list_contract[$key]['is_null'] = '1';
							} elseif ($val == 'price') {
								$fields_list_contract[$key]['setting'] = '';
					...
					if ($m == 'examine') {
						...
						if ($status) {
							...
							$examine_status = $examine_info['type'];
						}
						$examine_status_info = M('ExamineStatus')->where(array('status'=>$examine_status))->find();
						if ($examine_status_info['type'] == 1) {
							$this->ajaxReturn('','该审批类型已停用！',0);
						}
						$examine_setting = array();
						if ($examine_status_info['option'] == 1) {
							//查询审批人信息
							$examine_step_info = M('ExamineStep')->where(array('process_id'=>$examine_status))->order('order_id asc')->find();
							$examine_role_ids = $examine_step_info['role_id'];
							$examine_role_list = array();
							if ($examine_role_ids) {
								$examine_role_list = $d_user->get_full_name(explode(',',$examine_role_ids));
							...
							//自选
							$examine_role_ids = array();
							$examine_role_ids = $examine_info['examine_role_id'];
							$examine_role_list = array();
							if (count($examine_role_ids) == 1) {
								$examine_role_list = $d_user->get_full_name(explode(',',$examine_role_ids));
								$examine_role_list = array_values($examine_role_list);
						...
						//自定义字段
						$arr = array('owner_role_id','examine_role_id','content');
						$arr_examine = array('申请人'=>'owner_role_id','审批人'=>'examine_role_id','申请事由'=>'content');
						$fields_list_examine = array();
						foreach ($arr as $key=>$val) {
							
							foreach ($arr_examine as $ke=>$va) {
								if ($val == $va) {
									$field = $ke;
								}
							}
							$fields_list_examine[$key]['is_main'] = '1';
							$fields_list_examine[$key]['field'] = $val;
							$fields_list_examine[$key]['name'] = $field;

							$setting = '';
							if ($val == 'owner_role_id') {
								$fields_list_examine[$key]['setting'] = '';
								$fields_list_examine[$key]['form_type'] = 'user';
								$fields_list_examine[$key]['is_validate'] = '1';
								$fields_list_examine[$key]['is_null'] = '1';
							} elseif ($val == 'examine_role_id') {
								$fields_list_examine[$key]['setting'] = $examine_setting;
								$fields_list_examine[$key]['form_type'] = $examine_form_type;
								$fields_list_examine[$key]['is_validate'] = '1';
								$fields_list_examine[$key]['is_null'] = '1';
							} elseif ($val == 'content') {
								$fields_list_examine[$key]['setting'] = '';
								$fields_list_examine[$key]['form_type'] = 'textarea';
								$fields_list_examine[$key]['is_validate'] = '1';
								$fields_list_examine[$key]['is_null'] = '1';
							} else {
								$fields_list_examine[$key]['form_type'] = 'text';
								$fields_list_examine[$key]['setting'] = '';
								$fields_list_examine[$key]['is_validate'] = '0';
								$fields_list_examine[$key]['is_null'] = '0';
							}
							
							$default_value = '';
							switch ($val){
								case 'examine_role_id' : $default_value = $examine_role_ids;break;
							}
							$fields_list_examine[$key]['default_value'] = !empty($default_value) ? $default_value : '';
							$fields_list_examine[$key]['max_length'] = '';
							$fields_list_examine[$key]['is_unique'] = '0';
							$fields_list_examine[$key]['in_add'] = '1';
							$fields_list_examine[$key]['input_tips'] = '';
							// vdd($fields_list_examine[$key]);
						}
						vdd($fields_list_examine);
						if ($fields_list) {
							$fields_list = array_merge($fields_list_examine,$fields_list);
						} else {
							$fields_list = $fields_list_examine;
						}
					}
	...
	public function filelist() {
		if ($this->isPost()) {
			...
			$module_array = array('leads', 'customer', 'business', 'contract', 'log', 'product', 'task', 'finance', 'examine','invoice');
			if (!in_array($module, $module_array) || !$id) {
				...
				switch ($module) {
					case 'leads' : 
						...
						//判断权限
						$leads_info = M('Leads')->where(array('leads_id' => $leads_id))->find();
						$outdays = M('Config')->where('name="leads_outdays"')->getField('value');
						$outdate = empty($outdays) ? 0 : time()-86400*$outdays;	

						// 领取时间 大于等于 线索超出天数放入客户池
						if ($leads_info['have_time'] >= $outdate) {
							// 非客户池
							if (in_array(session('role_id'), $this->_permissionRes)) {
								$is_permission = 1;
							}
						} else {
							// 客户池
					...
					case 'examine' : 
						...
						break;
					case 'invoice' :
						$m_r_file = M('RFileInvoice');
						//判断权限
						$invoice_info = M('Invoice')->where('invoice_id = %d',$id)->find();
						if (in_array($invoice_info['create_role_id'],getPerByAction('invoice','view')) || session('?admin')){
							$is_permission = 1;
						}
						break;
					default : 
				...
				if ($is_permission == 1) {
					...
					foreach ($file_list as $k=>$v) {
						...
						$file_list[$k]['file_path'] = headPathHandle($v['file_path'], 1);
						$file_list[$k]['file_path_thumb'] = headPathHandle($v['file_path_thumb'], 1);
					}
	...
	public function loglist() {
		if ($this->isPost()) {
			$module = $_POST['module'] ? trim($_POST['module']) : '';
			$id = $_POST['id'] ? intval($_POST['id']) : '';
			$module_array = array('leads', 'customer', 'business', 'contract');
			if (!in_array($module, $module_array) || !$id) {
				$this->ajaxReturn('', '参数错误！', 0);
			} else {
				$is_permission = 0;
				...
				switch ($module) {
					...
					case 'business' : 
						...
						break;
					case 'contract':
						$m_r_log = M('RContractLog'); 
						//判断权限
						$below_ids = getPerByAction('contract','view');
						$business_info = M('contract')->where('contract_id = %d',$id)->find();
						if ($business_info && in_array($business_info['owner_role_id'], $below_ids)) {
							$is_permission = 1;
						}
						break;
				...
				if ($is_permission == 1) {
					...
					foreach ($log_list as $key=>$value) {
						...
						$log_list[$key]['owner']['thumb_path'] = headPathHandle($log_list[$key]['owner']['thumb_path']);
						$business_info = array();
						$business_id = '';
						if ($value['sign'] == 1) {
							//客户签到
							...
							if ($sign_info) {
								...
								$sign_img = array_map('headPathHandle', $sign_img, range(1, count($sign_img)));
							...
							if ($log_list[$key]['business_id']) {
								$contacts_id = $m_r_business_log->where('business_id = %d and log_id = %d', $log_list[$key]['business_id'], $value['log_id'])->getField('contacts_id');
							} else {
								$contacts_id = $m_r_customer_log->where('log_id = %d', $value['log_id'])->getField('contacts_id');
							}
							...
							$log_list[$key]['contacts'] = $contacts ?: null;
							$vp_where = array('module' => 'log', 'module_id' => $value['log_id']);
						}
						$log_list[$key]['visitor_plan'] = $m_visitor_plan->where($vp_where)->field('plan_time, content')->find();
					}
	...
	// {
	// 	$this->ajax(array('version' => C('VERSION') ), '', 2);
