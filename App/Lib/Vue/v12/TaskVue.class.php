...
class TaskVue extends Action {
	...
	public function index(){
		if ($this->isPost()) {
			...
			foreach ($task_list as $key=>$value) {
				...
				$task_list[$key]['about_roles_list'] = $about_roles_id ? $d_user->get_full_name($about_roles_id) : array();
				//子任务进度
				$schedule = 0;
				$sub_task_list = array();
	...
	public function view() {
		if ($this->isPost()) {
			...
			$d_user = D('User');
			$task_info['owner_roles_list'] = $task_info['owner_role_id'] ? $d_user->get_full_name(explode(',', $task_info['owner_role_id'])) : array();
			$task_info['about_roles_list'] = $task_info['about_roles'] ? $d_user->get_full_name(explode(',', $task_info['about_roles'])) : array();
			...
			foreach ($action_list as $k=>$v) {
				...
				$create_role_info = $temp_val[$v['role_id']];
			...
			foreach ($comment_list as $key => $value) {
				$temp_val = $d_user->get_full_name(array($value['send_role_id']));
				$creator_info = $temp_val[$value['send_role_id']];
				$comment_list[$key]['user_name'] = $user_info['full_name'];
				$comment_list[$key]['img'] = headPathHandle($user_info['thumb_path']);
				...
				foreach ($comment_list_child as $childkey => $childvalue) {
					$temp_val = $d_user->get_full_name(array($childvalue['send_role_id']));
					$creator_child_info = $temp_val[$childvalue['send_role_id']];
					$comment_list_child[$childkey]['childimg'] = headPathHandle($creator_child_info['thumb_path']);
