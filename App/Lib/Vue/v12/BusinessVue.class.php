	...
	public function index() {
		if ($this->isPost()) {
			...
			if (intval($_POST['customer_id'])) {
				...
				//排除已签订合同的商机
				$business_ids = M('Contract')->where(array('customer_id'=>intval($_POST['customer_id']),'is_deleted'=>0))->getField('business_id',true);
				//过滤空商机
	...
	public function edit() {
		if ($this->isPost()) {
			...
			if ($d_business->create($params) && $d_business_data->create($params) !== false) {
				...
				if ($d_business_data->where(array('business_id'=>$business_id))->count() > 0) {
		...
		if ($this->isPost()) {
			...
			if ($advance_list) {
				...
				foreach ($advance_list as $k=>$v) {
					...
					$advance_list[$k]['img'] = headPathHandle($user_info['thumb_path']);
				}
			}
