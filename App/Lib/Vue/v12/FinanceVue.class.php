...
class FinanceVue extends Action{
	...
	**/
	public function _initialize(){
		...
		if (!in_array($this->type,array('receivables','receivingorder','payables','paymentorder'))) {
	...
	public function getCode() {
		if ($this->isPost()) {
			switch ($this->type) {
				...
				break;
				case 'receivingorder' : case 'paymentorder':
					//收款账户信息
				...
				case 'payables' :
					...
					$number = $payables_custom.date('Ymd').'-'.$payables_max_code;
					$data['data'] = $number ? $number : '';
					$data['status'] = 1;
					$data['info'] = 'success';
					$this->ajaxReturn($data,'JSON');
			...
			switch ($this->type) {
				...
				case 'payables' :
					...
					if ($m_payables->create()) {
						$m_payables->name = $_POST['name'] ? trim($_POST['name']) : $this->ajaxReturn('','应付款编号已存在，请修改后重新提交',0);
						$m_payables->pay_time = $_POST['pay_time'] ? : time();
				...
				case 'paymentorder' :
					...
					if ($money_total > $payables_info['price']) {
						$this->ajaxReturn('',L('EXCEED THE AMOUNT OF PAYMENT'),0);
					}
	...
	public function index(){
		if ($this->isPost()) {
			...
			if ($_POST['order_field'] && $_POST['order_type']) {
				$order = $this->type.'.'.trim($_POST['order_field']).' '.trim($_POST['order_type']).','.$this->type.'.update_time desc';
			}
			$p = isset($_POST['p']) ? intval($_POST['p']) : 1 ;
			$by = trim($_POST['by']) ? trim($_POST['by']) : 'all';
			...
			switch ($this->type) {
				...
				case 'receivables' :
					...
					if(!$_POST['field']){
						...
						foreach($_POST as $kd => $vd){
							if ($kd != 'act' && $kd != 'content' && $kd != 'p' && $kd != 'search' && $kd != 't' && $kd != 'type' && $kd != 'by' && $kd != 'listrows' && $kd != 'r_status' && $kd != 'token' && $kd != 'order_field' && $kd != 'order_type') {
								if(in_array($kd,array('create_time','update_time','pay_time'))){
									...
									if(!empty($vd['value'])){
										if ($vd['value'] == '3') {
											$where[$this->type .'status'] = array('egt',0);
										} else {
											$where[$this->type .'status'] = $vd['value'];
										}
									}
								}else{
									if(is_array($vd)) {
					...
					$list = $d_receivables->where($where)->order($order)->page($p.',10')->field('receivables_id,name,pay_time,price,customer_name,customer_id')->select();
					$m_receivingorder = M('Receivingorder');
					foreach($list as $k=>$v){
						$money += $v['price'];
					...
					if($p == 1 && $_POST['search'] == ''){
						$status_setting = array(
											'0'=>array('key'=>3,'value'=>'全部'),
											'1'=>array('key'=>0,'value'=>'未收款'),
											'2'=>array('key'=>1,'value'=>'部分已收'),
											'3'=>array('key'=>2,'value'=>'已收')
											);
						$fields_list = array(
							'0'=>array('field'=>'name','form_type'=>'text','input_tips'=>'','name'=>'应收款编号','setting'=>''),
							'1'=>array('field'=>'customer_name','form_type'=>'text','input_tips'=>'','name'=>'客户名称','setting'=>''),
							'2'=>array('field'=>'contract_name','form_type'=>'text','input_tips'=>'','name'=>'合同编号','setting'=>''),
							'3'=>array('field'=>'price','form_type'=>'text','input_tips'=>'','name'=>'收款金额','setting'=>''),
							'4'=>array('field'=>'pay_time','form_type'=>'datetime','input_tips'=>'','name'=>'应收款时间','setting'=>''),
							'5'=>array('field'=>'receivables_status','form_type'=>'box','input_tips'=>'','name'=>'状态','setting'=>$status_setting),
							'6'=>array('field'=>'owner_role_id','form_type'=>'user','input_tips'=>'','name'=>'负责人','setting'=>''),
							'7'=>array('field'=>'creator_role_id','form_type'=>'user','input_tips'=>'','name'=>'创建人','setting'=>''),
							'8'=>array('field'=>'create_time','form_type'=>'datetime','input_tips'=>'','name'=>'创建时间','setting'=>''),
							'9'=>array('field'=>'update_time','form_type'=>'datetime','input_tips'=>'','name'=>'修改时间','setting'=>'')
						);
					}
				...
				case 'receivingorder' :
					...
					if(!$_POST['field']){
						foreach($_POST as $kd => $vd){
							if ($kd != 'act' && $kd != 'content' && $kd != 'p' && $kd != 'search' && $kd != 't' && $kd != 'type' && $kd != 'by' && $kd != 'listrows' && $kd != 'token' && $kd != 'order_field' && $kd != 'order_type') {
								if(in_array($kd,array('create_time','update_time','pay_time'))){
									$where[$kd] = field($vd['value'], $vd['condition']);
					...
					$list = $d_receivingorder->where($where)->order($order)->page($p.',10')->field('receivingorder_id,name,pay_time,money,receivables_id,status,owner_role_id,receipt_account,receipt_bank,company,description,customer_id')->select();
					$m_user = M('User');
					...
					foreach($list as $k=>$v){
						...
						$customer_name = '';
						$customer_name = $m_customer->where(array('customer_id'=>$v['customer_id']))->getField('name');
					...
					if($p == 1 && $_POST['search'] == ''){
						$status_setting = array(
											'0'=>array('key'=>3,'value'=>'全部'),
											'1'=>array('key'=>0,'value'=>'待审核'),
											'2'=>array('key'=>1,'value'=>'已通过'),
											'3'=>array('key'=>2,'value'=>'已拒绝')
											);
						$fields_list = array(
							'0'=>array('field'=>'name','form_type'=>'text','input_tips'=>'','name'=>'收款单号','setting'=>''),
							'1'=>array('field'=>'money','form_type'=>'text','input_tips'=>'','name'=>'收款金额','setting'=>''),
							'2'=>array('field'=>'pay_time','form_type'=>'datetime','input_tips'=>'','name'=>'收款时间','setting'=>''),
							'3'=>array('field'=>'status','form_type'=>'text','input_tips'=>'','name'=>'状态','setting'=>$status_setting),
							'4'=>array('field'=>'owner_role_id','form_type'=>'user','input_tips'=>'','name'=>'负责人','setting'=>''),
							'5'=>array('field'=>'creator_role_id','form_type'=>'user','input_tips'=>'','name'=>'创建人','setting'=>''),
				...
				case 'payables' :
					...
					//应付款状态
					if ($_POST['status']['value'] == 3) {
						$where['payables.status'] = array('eq',0);
					}

					$count = $d_payables->where($where)->count();
					$list = $d_payables->where($where)->order($order)->page($p.',10')->select();
					...
					if($p == 1 && $_POST['search'] == ''){
						$status_setting = array(
											'0'=>array('key'=>3,'value'=>'全部'),
											'1'=>array('key'=>0,'value'=>'未付款'),
											'2'=>array('key'=>1,'value'=>'部分已付'),
											'3'=>array('key'=>2,'value'=>'已付')
											);
						$fields_list = array(
							'0'=>array('field'=>'name','form_type'=>'text','input_tips'=>'','name'=>'应付款编号','setting'=>''),
							'1'=>array('field'=>'customer_name','form_type'=>'text','input_tips'=>'','name'=>'客户名称','setting'=>''),
							'2'=>array('field'=>'contract_name','form_type'=>'text','input_tips'=>'','name'=>'合同编号','setting'=>''),
							'3'=>array('field'=>'price','form_type'=>'text','input_tips'=>'','name'=>'付款金额','setting'=>''),
							'4'=>array('field'=>'pay_time','form_type'=>'datetime','input_tips'=>'','name'=>'应收款时间','setting'=>''),
							'5'=>array('field'=>'payables_status','form_type'=>'box','input_tips'=>'','name'=>'状态','setting'=>$status_setting),
							'6'=>array('field'=>'owner_role_id','form_type'=>'user','input_tips'=>'','name'=>'负责人','setting'=>''),
							'7'=>array('field'=>'creator_role_id','form_type'=>'user','input_tips'=>'','name'=>'创建人','setting'=>''),
							'8'=>array('field'=>'create_time','form_type'=>'datetime','input_tips'=>'','name'=>'创建时间','setting'=>''),
							'9'=>array('field'=>'update_time','form_type'=>'datetime','input_tips'=>'','name'=>'修改时间','setting'=>'')
				...
				case 'paymentorder' :
					...
					if ($_POST['payables_id']) {
						$where['payables_id'] = intval($_POST['payables_id']);
						$payables_price = M('Payables')->where(array('payables_id'=>intval($_POST['payables_id'])))->getField('price');
					}

					$count = $d_paymentorder->where($where)->count();
					$list = $d_paymentorder->where($where)->order($order)->page($p.',10')->select();

					$paymentorder_money = 0; //已付款金额
					$un_payables_money = 0; //未付款金额
					foreach ($list as $k=>$v) {
						...
						if($v['status'] == 1){
							//计算已结账状态的金额
							$paymentorder_money += $v['money'];
						}
						//账户名称
						$receipt_bank = '';
						$receipt_bank = $m_bank_account->where(array('account_id'=>$v['bank_account_id']))->getField('open_bank');
						$list[$k]['receipt_bank'] = $receipt_bank ? : '';
					}

					//查询应收款下回款，返回合计
					if ($_POST['payables_id']) {
						$un_payables_money = ($payables_price - $paymentorder_money) < 0 ? 0 : ($payables_price - $paymentorder_money);
					}
					$data['paymentorder_money'] = $paymentorder_money;
					$data['un_payables_money'] = $un_payables_money;
					...
					if($p == 1 && $_POST['search'] == ''){
						...
						$fields_list = array(
							'0'=>array('field'=>'name','form_type'=>'text','input_tips'=>'','name'=>'付款单编号','setting'=>''),
							'1'=>array('field'=>'money','form_type'=>'text','input_tips'=>'','name'=>'付款金额','setting'=>''),
							'2'=>array('field'=>'pay_time','form_type'=>'datetime','input_tips'=>'','name'=>'付款时间','setting'=>''),
							'3'=>array('field'=>'status','form_type'=>'text','input_tips'=>'','name'=>'状态','setting'=>$status_setting),
							'4'=>array('field'=>'owner_role_id','form_type'=>'user','input_tips'=>'','name'=>'负责人','setting'=>''),
							'5'=>array('field'=>'creator_role_id','form_type'=>'user','input_tips'=>'','name'=>'创建人','setting'=>''),
							'6'=>array('field'=>'create_time','form_type'=>'datetime','input_tips'=>'','name'=>'创建时间','setting'=>''),
			...
			switch ($this->type) {
				...
				case 'receivingorder' :
					...
					if (!$info['creator_role_id']) {
						...
						$info['creator_name'] = $info['owner_name'];
					}
					//获取操作权限
					$data['permission'] = permissionlist(MODULE_NAME,$info['owner_role_id'],'receivingorder');
					//审核权限
	...
	public function check_list(){
		if ($this->isPost()) {
			...
			$d_user = D('User');
			$temp_val = $d_user->get_full_name(array($res_info['owner_role_id']));
			$owner_info = $temp_val[$res_info['owner_role_id']];
			if ($res_info['examine_role_id']) {
				$temp_val = $d_user->get_full_name(array($res_info['examine_role_id']));
				$examine_info = $temp_val[$res_info['examine_role_id']];
			} else {
				$examine_info = array();
			}
