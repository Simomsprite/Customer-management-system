...
class InstallAction extends Action {	
	...
	public function index(){
		...
		if (!file_exists(getcwd() . "/Public/sql/pdcrm.sql")) {
		...
		if ($_POST['submit']) {
			...
			if (empty($warnings)) {
				...
				if(file_put_contents(CONF_PATH . "db.php", $db_config_str)){
					$db = M();
                    $sql = file_get_contents(getcwd() . "/Public/sql/pdcrm.sql");
                    $sql = str_replace("pdcrm_", C('DB_PREFIX'), $sql); 
                    $sql = str_replace("http://demo.xxx.com", __ROOT__, $sql);
					$sql = str_replace("\r\n", "", $sql); 
					$queries = explode(";\n", $sql); 
			...
			if( $dir_handle = opendir($dir) ){
				while (false !== ( $file_name = readdir($dir_handle)) ) {
					if($file_name=='.' or $file_name =='..'){
						continue;
					} elseif ($file_name != "pdcrm.sql" && strpos($file_name,'.sql')){
