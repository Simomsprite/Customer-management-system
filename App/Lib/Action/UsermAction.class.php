...
class UsermAction extends Action {
	...
	public function _initialize(){
		$action = array(
			...
			'allow'=>array('logout','role_ajax_add','getrolebydepartment','dialoginfo','edit', 'listdialog', 'mutilistdialog', 'getrolelist', 'getpositionlist','changecontent','getactionauthority','department','userimg','contacts','yanchong','editpassword','getpositionlistbydepartment','copy_auth','editpwd')
		);
	...
	**/
		...
		if (session('?name')){
            ...
            if((!isset($_POST['name']) || $_POST['name'] =='')||(!isset($_POST['password']) || $_POST['password'] =='')){
                ...
                if ($user['password'] == md5(trim($_POST['password']) . $user['salt'])) {
					if (-1 == $user['status']) {
                        echo 3;die;//您的账号未通过审核，请联系管理员
					} elseif (0 == $user['status']) {
                        echo 4;die;//您的账号正在审核中，请耐心等待
						...
						if (!is_array($role) || empty($role)) {
							echo 6;die;//系统没有给您分配任何岗位，请联系管理员
						} else {
							if(3 == $user['status']){
							...
							if($user['category_id'] == 1){
								...
								if ($key_file) {
									...
									$days_remaining = '100';
									if ($authorize_setting['NUM'] <= 5 && !empty($authorize_setting['OPENTIME']) && $authorize_setting['OPENTIME'] > '20171116') {
										if ($authorize_setting['NUM'] > 2) {
								...
								session('days_remaining', $days_remaining);
							}
	...
	public function lostpw() {
		if($this->isPost()){
			if ($_POST['name'] || $_POST['email']){
				...
				C(F('smtp'),'smtp');
				import('@.ORG.Mail');
				$url = U('userm/resetpw', array('user_id'=>$info['user_id'], 'verify_code'=>$verify_code),'','',true);
				$content = L('FIND_PASSWORD_EMAIL' ,array($_POST['name'] , $url));
				if (SendMail($info['email'],L('FIND_PASSWORD_LINK'),$content,L('pdcrm_ADMIN'))){
	...
	public function resetpw(){
		...
		if (is_array($user) && !empty($user)) {
			if ((time()-$user['lostpw_time'])>86400){
				alert('error', L('LINK_DISABLE_PLEASE_FIND_PASSWORD_AGAIN'),U('userm/lostpw'));
				...
				if ($_REQUEST['password']) {
					...
					alert('success', L('EDIT_PASSWORD_SUCCESS_PLEASE_LOGIN'), U('userm/login'));
				} else {
					$this->alert = parseAlert();
	...
	public function listDialog() {
		...
		foreach ($role_list as $key => $value) {
			$role_list[$key]['img'] = $value['img'] ? $value['img'] : '__PUBLIC__/img/avatar_default.png';
		}
	...
	public function mutiListDialog(){
		...
		if($_GET['by'] == 'task'){
			...
			//有审核权限的
			$url = getCheckUrlByAction('examine', 'add_examine');
			$position_ids = M('Permission')->where(array('url'=>$url))->getField('position_id',true);
			$position_ids[] = 1;
		} elseif ($_GET['by'] == 'contract') {
			//有审核权限的
			$url = getCheckUrlByAction('contract', 'check');
			$position_ids = M('Permission')->where(array('url'=>$url))->getField('position_id',true);
			$position_ids[] = 1;
		}else{
		...
		foreach($departments_list as $k=>$v){
			...
			if ($_GET['by'] == 'examine' || $_GET['by'] == 'contract') {
				$where['position.position_id'] = array('in',$position_ids);
			}
			$where['status'] = 1;
			$where['position.department_id'] = $v['department_id'];
			$roleList = $d_role->where($where)->select();
			foreach ($roleList as $ke => $val) {
				$roleList[$ke]['img'] = $roleList[$ke]['thumb_path'] = headPathHandle($val['thumb_path']);
			}
	...
	public function userimg() {
		if ($this->isAjax() && $_FILES['blob']['error'] != 4) {
	        ...
	        if ($dir) {
	        	//设置文件名称
	        	$file_name = date('YmdHis').rand(1000, 9999).'.'.getExtension($_FILES['blob']['name']);
	            //阿里云OSS的文件名称
	            $object = $dir.$file_name;
	            $res = $d_file->ossUpload($object, $_FILES['blob']['tmp_name']);
	            if ($res) {
	            	//保存 上传的图片
					$img_thumb['img'] = $object; 
					$img_thumb['thumb_path'] = $object.'?x-oss-process=image/resize,m_fixed,h_100,w_100';
	            } else {
	            	$ajaxRet['status'] = 5;
					$ajaxRet['msg'] = '头像设置失败！';
					$this->ajaxReturn($ajaxRet);
                ...
                /*if ($info[0]['size']>20000000){
                    $ajaxRet['status'] = 9;
                    $ajaxRet['msg'] = '头像大小不得大于200kb！';
                    $this->ajaxReturn($ajaxRet);
                }*/
			...
			if($result){
				if (strpos($server_name, 'pdcrm.com')) {
					//删除原图
	            	$d_file->ossDelete(array($thumb_old));
	            	session('user_img', $d_file::FILE_URL.'/'.$img_thumb['img']);
				} else {
					@unlink($thumb_old);
					session('user_img', $img_thumb['img']);
				}
    ...
    public function userimgs(){
	    ...
	    $m_user=M('User');
	    $user_info=$m_user->where('user_id='.$user_id)->find();
	...
	public function editPassword(){
		if (session('?admin')) {
			// demo2体验站，管理员密码禁止随意修改
			$this->ajaxReturn('','禁止修改管理员密码！',0);
		}
		$m_user = M('user');
        if($this->isAjax()) {
            //验证是否是本人修改
	...
	public function edit(){
		if ($this->isPost()) {
            if(!session('?admin') && (session('user_id') != $_POST['user_id'])){
            	if(!checkPerByAction(MODULE_NAME,ACTION_NAME)){
            		if(checkPerByAction('user','index')){
                        $this->ajaxReturn('', 'YOU_DO_NOT_HAVE_THIS_RIGHT', 0);
            		}else{
                        $this->ajaxReturn('', 'YOU_DO_NOT_HAVE_THIS_RIGHT', 0);
            		}
				}
            ...
            if ($_POST['telephone'] && !ereg('^1[345789][0-9]{9}$', $_POST['telephone'])){
                $data = array(
                    'data'=>'',
                    'msg'=>'手机格式不正确',
                    'status'=>'0'
            ...
            if (strtotime($_POST['birthday']) >= $beginToday){
                $data = array(
                    'data'=>'',
                    'msg'=>'出生日期不能大于今天',
                    'status'=>'0'
            ...
            if(!session('?admin') && (session('user_id') != $user_id)){
				if(!checkPerByAction(MODULE_NAME,ACTION_NAME)){
					if(checkPerByAction('user','index')){
						alert('error',L('YOU_DO_NOT_HAVE_THIS_RIGHT'),U('userm/index'));
            		}else{
			...
			$user['img'] = headPathHandle($user['img']);
			$user['thumb_path'] = headPathHandle($user['thumb_path']);
			...
			$where['is_validate'] = 1;
			$where['is_null'] = 1;
			$fields = M('fields')->where($where)->field('field')->select();
			if($fields !=''){
			    $this->assign('need_validata',$fields);
            }
			//判断是否启用客户数限制
			...
			$user['type_name'] = $user_type_list[$user['type'] - 1];
//			dd($user);
			$this->user = $user;
	...
	public function dialogInfo(){
		...
		$user['img'] = $user['thumb_path'] = headPathHandle($user['thumb_path']);
		$user[role] = $role;
		$this->user = $user;
		$this->categoryList = M('user_category')->select();
		$this->alert = parseAlert();
		...
		if (is_array($user) && !empty($user)) {
			if (md5(md5($user['reg_time']) . $user['salt']) == $verify_code) {
				if ($_REQUEST['password']) {
					...
					alert('success', L('SET_PASSWORD_SUCCESS_PLEASE_LOGIN'), U('userm/login'));
				} else {
					$this->alert = parseAlert();
	...
	public function index(){
		...
		$this->by_parameter = str_replace('status='.$status, '', implode('&', $params));
		...
		foreach($user_list as $k=>$v){
			...
			$user_list[$k]['img'] = headPathHandle($v['img']);
			$user_list[$k]['thumb_path'] = headPathHandle($v['thumb_path']);
		}
	...
	public function department_delete(){
		if(!session('?name') || !session('?user_id')){
			alert(L('PLEASE_LOGIN_FIRSET'),U('Userm/login/'), 'error');
		}
	...
	public function position_edit(){
		...
		foreach($position_list as $key=>$value){
			if($value['parent_id'] == intval($_GET['id']) || $value['position_id'] == intval($_GET['id'])){
	...
	public function user_add(){
		...
		if(!$user_custom)  $user_custom = 'pd_crm';
...
/*插入下级*/
	...
	public function user_authorize(){
		if($this->isAjax() && $this->isPost()){
			...
			$m_permission = M('Permission');
			$copy_position_id = intval($_GET['copy_position_id']);
			if ($copy_position_id > 0) {
				//权限复制，即查询出要复制的岗位权限信息
				$temp_array = $m_permission->where('position_id = %d', $copy_position_id)->select();
			...
			$this->per_list = D('User')->authorize();
// dd($this->per_list);
			
			$this->per_type = y_array_column($this->per_list, 'name');
// dd($this->per_type);
	...
	public function getRoleList(){
		...
		$roleList = getUserByRoleIdArray($idArray);
		$this->ajaxReturn($roleList, '', 1);
	}
	...
	public function contacts(){
		...
		if($type == 1){
			...
			foreach($department_list as $k=>$v){
				...
				foreach ($department_role_list as $key => $val) {
					$department_role_list[$key]['img'] = $department_role_list[$key]['thumb_path'] = headPathHandle($val['thumb_path']);
				}
			...
			foreach ($role_list as $key => $val) {
				$role_list[$key]['img'] = $role_list[$key]['thumb_path'] = headPathHandle($val['thumb_path']);
			}
			$count = $d_role->where($where)->count();
			$Page = new Page($count,$listrows);// 实例化分页类 传入总记录数和每页显示的记录数
			$show = $Page->show();// 分页显示输出
	 ...
	 * 用户登录超时弹出模态框
	 * @param 
	 * @author xiaoxu gengxiaoxu@pdcrm.com
	 * @return 
	 */
	...
	public function copy_auth(){
		$position_id = intval($_GET['position_id']);
		$department_id = M('position')->where('position_id = %d', intval($_GET['position_id']))->getField('department_id');
