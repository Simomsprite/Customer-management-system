...
class FileAction extends Action{
	...
	public function _initialize(){
		$action = array(
			'permission'=>array('aliosskeycallback', 'aliOssKeyCallBack'),
			'allow'=>array('editor', 'add', 'delete','filedownload','add_file','addlogfile','getfiles','filedel', 'check_max_size', 'aliosskey', 'aliOssKey')
		);
		...
		if($this->isAjax()){
			...
			if($file_id){
				...
				switch ($r_module) {
					...
					case 'market' : $r = M('RFileMarket'); break;
				}
				if ($r) {
	...
	public function add(){
		...
		if(!empty($_FILES)){
			...
			if(is_array($info[0]) && !empty($info[0])){
				...
				$data['name'] = str_replace('&', '_', $info[0]['name']);
			...
			$this->file_max_size = D('File')->getMaxSize();
			$this->file_max_size_num = (int) $this->file_max_size * 1024 *1024; 
			$this->r = $_GET['r'];
			...
			if (isset($_GET['r']) && isset($_GET['id'])) {
				...
				if (is_array($file) && ($file['role_id'] == session('role_id') || session('?admin'))){
					if ($m_r->where('file_id = %d',$_GET['id'])->delete()) {
						...
						/**
						 * 判断如果是文件上传至OSS，则需要先删除OSS上保存的文件
						 * oss 情况无需生成缩略图，所以不用处理 $file['file_path_thumb']
						*/ 
						if ($file['oss'] == 1) {
							$d_file = D('File');
							$file_path_list = array($file['file_path']);
							$res = $d_file->ossDelete($file_path_list);
							if ($res['status'] === false) {
								$this->ajaxReturn('', $d_file->msg, 0);
							}
	...
	public function aliOssKey()
	{
		...
		// 回调地址
		$callbackUrl = 'http://' . $server_name . $_SERVER["SCRIPT_NAME"] . '?m=file&a=aliOssKeyCallBack';
	...
	public function aliOssKeyCallBack()
	{
		...
		if ($file_id = $m_file->add($data)) {
			if ($module == '' || $m_id == '' || $r_file_module == '') {
				$this->ajaxReturn(array('file_id' => $file_id, 'msg' => '上传成功！', 'status' => 1));
			}
			$temp['file_id'] = $file_id;
			$temp[$module . '_id'] = $m_id;
			if ($r_file_module->add($temp)) {
				...
				// 删除oss文件
				$this->ajaxReturn(array('msg' => '上传失败！', 'status' => 0));
			}
		} else {
			$this->ajaxReturn(array('msg' => '上传失败！', 'status' => 0));
		}
	...
	{
		$new_file_size = $_GET['new_file_size'];
		$d_file = D('File');
		$res = $d_file->checkOssMaxSize($new_file_size);
		if ($res) {
			$this->ajaxReturn(array('msg' => $d_file->msg, 'status' => 0));
		} else {
			$this->ajaxReturn(array('msg' => '', 'status' => 1));
		}
	}
