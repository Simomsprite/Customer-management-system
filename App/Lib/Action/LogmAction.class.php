...
*
**/
class LogmAction extends Action{
	/**
	*用于判断权限
	*@permission 无限制
	*@allow 登录用户可访问
	...
	**/
	...
	public function myCommont(){
		if($this->isAjax()){
			...
			$content = strip_tags($content, "<img>");	// 过滤非法标签
		...
		if($this->isPost()){
			if($_POST['r']){
				...
				if($log_id = $m_log->add()){
					if (strtotime($_POST['nextstep_time'])) {
						//关联日程
						$event_id = dataEvent(trim($_POST['content']),strtotime($_POST['nextstep_time']),$module,$model_id);
					}
					//保存为沟通模板
					...
					if ($_POST['contacts_id']) {
						$data['contacts_id'] = intval($_POST['contacts_id']);
					}
					if($m_r->add($data)){
						//相关模块更新时间
						if($module == 'customer' || $module == 'business' || $module == 'leads'){
							...
							if($module == 'business'){
								...
								$customer_data['grade'] = (int) $_POST['grade'];
								if ($customer_data['grade'] == 0) {
									unset($customer_data['grade']);
								}
								...
								if (strtotime($_POST['nextstep_time'])) {
									...
									if (isset($_POST['join_visitor_plan']) && $_POST['join_visitor_plan'] == 1) {
										$visitor_plan_content = $_POST['visitor_plan_content'] ?: $_POST['content'];
										$data = array(
											'event_id' => $event_id,
											'plan_time' => $customer_data['nextstep_time'],
											'content' => $visitor_plan_content
										);
							...
							if ($visitor_plan_id = (int) $_POST['visitor_plan_id']) {
								$vp_res = D('VisitorPlan')->finishPlan(array('id' => $visitor_plan_id, 'module' => 'log', 'module_id' => $log_id));
							}
						}
						$d_user = D('User');
						$temp_val = $d_user->get_full_name(array(session('role_id')));
						// $ajax_data['owner'] = $m_user->where('role_id = %d', session('role_id'))->field('full_name,role_id,thumb_path')->find();
						$ajax_data['owner'] = $temp_val[session('role_id')];
						$ajax_data['vp_res'] = $vp_res;
			...
			if ($_GET['module'] == 'customer') {
				...
				$this->visitor_plan_list = D('VisitorPlan')->getList($v_p_where);	// 完成和延期
				...
				if ($log_id) {
					...
					$contacts_id = $m_r_customer_log->where('customer_id = %d and log_id = %d', intval($_GET['id']), $log_info['log_id'])->getField('contacts_id');
					...
					$log_info += $contacts ?: array();
				}
				$this->star = M('customer')->where('customer_id = %d', $_GET['id'])->getField('grade');
				$this->log_info = $log_info ? $log_info : array();
				$this->log_count = $log_count ? $log_count : 0;

				// 联系人信息
				$where_contacts['customer.customer_id'] = intval($_GET['id']);
				$this->contacts_list = D('ContactsView')->where($where_contacts)->select();
			}
	...
	public function index(){
		...
		if ($_REQUEST["field"]) {
			...
			if (isset($where['subject'])) {
				$map['subject'] = array('LIKE', '%' . $where['subject'][1] . '%');
				$map['content'] = array('LIKE', '%' . $where['subject'][1] . '%');
				$map['_logic'] = 'OR';
				$where['_complex'] = $map;
				unset($where['subject']);
			}
		...
		// 高级搜索
		$fields_search = array();
		if ($_REQUEST['search'] == 'true' || isset($_REQUEST['create_date'])) {
			$fields = array('role_id', 'category_id', 'subject', 'create_date', 'content');
			foreach ($_GET as $key => $val) {
				if (in_array($key, $fields)) {
					$val['field'] = $key;
					$fields_search[$key] = $val;
					if ($key == 'subject') {
						switch ($val['condition']) {
							case "is" : $where[$key] = array('eq', $val['value']);break;
							case "isnot" :  $where[$key] = array('neq', $val['value']);break;
							case "contains" :  $where[$key] = array('like','%'. $val['value'].'%');break;
							case "not_contain" :  $where[$key] = array('notlike','%'. $val['value'].'%');break;
							case "start_with" :  $where[$key] = array('like', $val['value'].'%');break;
							case "end_with" :  $where[$key] = array('like','%'. $val['value']);break;
							case "is_empty" :  $where[$key] = array('eq','');break;
							case "is_not_empty" :  $where[$key] = array('neq','');break;
							case "gt" :  $where[$key] = array('gt', $val['value']);break;
							case "egt" :  $where[$key] = array('egt', $val['value']);break;
							case "lt" :  $where[$key] = array('lt', $val['value']);break;
							case "elt" :  $where[$key] = array('elt', $val['value']);break;
							case "eq" : $where[$key] = array('eq', $val['value']);break;
							case "neq" : $where[$key] = array('neq', $val['value']);break;
							case "between" : $where[$key] = array('between', array($val['value']-1,$val['value']+86400));break;
							case "nbetween" : $where[$key] = array('not between', array($val['value'],$val['value']+86399));break;
							case "tgt" :  $where[$key] = array('gt', $val['value']+86400);break;
							default : $where[$key] = array('eq', $val['value']);
						}
					} else if ($key == 'create_date') {
						$create_date = array('between', array(strtotime($val['start']), strtotime($val['end'])));
						$where['create_date'] = $create_date;
					} else {
						$where[$key] = field($val['value'], $val['condition']);
					}
				}
			}
		}
	...
	**/
	 ...
	 * 日志详情加载（列表）
	 * @param 
	 * @author xiaoxu gengxiaoxu@xxx.com
	 * @return 
	 */
	...
	**/
	...
	public function commun_list(){
		...
		if ($_GET['sdate'] && $_GET['edate']) {
			$search_date = array('between',array(intval($_GET['sdate']),intval($_GET['edate'])));
		} else {
		...
		if (!empty($module) && !empty($module_id)) {
			switch ($module) {
				...
				// 访客计划完成关联 log sign
				case 'log':
					$log_list[] = $m_log->where(array('log_id' => $module_id))->find();
					$is_permission = 1;
					break;
				case 'sign':
					$log_id = $m_sign->where(array('sign_id' => $module_id))->getField('log_id');
					$log_list[] = $m_log->where(array('log_id' => $log_id))->find();
					$is_permission = 1;
					break;
				default : 
					$m_r_file = ''; 
			...
			if ($is_permission == 1) {
				...
				foreach ($log_list as $key=>$value) {
					...
					// 相关联系人
					$contacts_id = $m_r_customer_log->where('customer_id = %d and log_id = %d', $module_id, $value['log_id'])->getField('contacts_id');
					$contacts = $m_contacts->where('contacts_id = %d', $contacts_id)->field('contacts_id,name as contacts_name,telephone as contacts_phone')->find();
					$log_list[$key] += $contacts ?: array();
				}
			} else {
