...
class MessageAction extends Action{
	...
	public function tips(){
		...
		$new_num['message_time'] = $message_time ? getTimeBySec(time()-$message_time) : '0分钟';
		...
		$contract_examine_role_ids = array();
		...
		// 待审核的发票
		$new_num['todo_num'] += $new_num['check_invoice_num'] = $d_message->checkInvoiceNum();
		...
		if($message_content){
			foreach($message_content as $k=>$v){
				...
				if($v['file_id']){
					...
					$message_content[$k]['file_info']['file_path'] = headPathHandle($message_content[$k]['file_info']['file_path']);
					$message_content[$k]['file_info']['file_path_thumb'] = headPathHandle($message_content[$k]['file_info']['file_path_thumb']);
				}
		...
		if(intval($_GET['to_role_id'])){
			...
			foreach($all_message_list as $k=>$v){
				...
				if($v['file_id']){
					...
					$all_message_list[$k]['file_info']['file_path'] = headPathHandle($all_message_list[$k]['file_info']['file_path']);
					$all_message_list[$k]['file_info']['file_path_thumb'] = headPathHandle($all_message_list[$k]['file_info']['file_path_thumb']);
				}
			}
			//标记为已读状态
			$read_list = array();
