...
*
**/
class LeadsmAction extends Action{
	...
	**/
	public function _initialize(){
		$action = array(
			'permission'=>array(),
			'allow'=>array('transform','checkinfo','changecontent','getaddchartbyroleid','getownchartbyroleid','listDialogs','check','receive','fenpei','batchreceive', 'assigndialog', 'batchassign', 'revert', 'validate', 'remove','excelimportdownload','excelimportact','change_customer','field_save','sendsms')
		);
	...
	**/
	public function index(){
		...
		if(trim($_GET['act'] == 'sms')){
			...
			//加入联系人和联系电话
			$where2=array(
			    'mobile'=>array('like',$where['name'][1]),
                'contacts_name'=>array('like',$where['name'][1]),
                '_complex'=>$where,
                '_logic'=>'or'
            );

			$this->_where = http_build_query($where);
			$list = $m_leads->where($where2)->page($p.','.$listrows)->order($order)->select();

			//SELECT * FROM `pd_leads` WHERE ( `mobile` LIKE null ) OR ( `contacts_name` LIKE null )
            // OR (  ( `have_time` >= 1570703460 ) AND ( `is_deleted` = 0 ) AND ( `is_transformed` = 0 )
            // AND ( `owner_role_id` = '1' ) ) ORDER BY create_time desc LIMIT 0,15
				...
				if($m_leads_data->create()!==false){
					...
					if ($leads_id = $m_leads->add()) {
						...
						if($_POST['submit'] == L('SAVE')) {
							alert('success', L('LEADS_ADD_SUCCESS'), U('leadsm/index'));
						} else {
							alert('success', L('LEADS_ADD_SUCCESS'), U('leadsm/index'));
						}
					} else {
	...
	**/
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($m_leads->create()) {
                if ($m_leads_data->create() !== false) {
                    ...
                    $m_leads->address = implode(chr( 10),$_POST['address']);
                    $m_leads->update_time = time();
                    $a = $m_leads->where('leads_id='.$leads_id)->save();
                    if ($a !== false) {
                        actionLog($_REQUEST['leads_id']);
                        alert('success', L('LEADS_MODIFIED_SUCCESSFULLY'),  U('leadsm/index'));
                    } else {
	...
	public function edit(){
	    $leads_id = $this->_get('id','intval', intval($_GET['leads_id']));
		if(!$leads_id){
     ...
     *线索详情页面
     */
    public function leadsDetail(){
        $leads_id=I('get.leads_id/d');
        if ($leads_id!=" ") {
            $m_leads = M('leads');
            $leads = $m_leads->where('leads_id=' . $leads_id)->find();
            if ($leads) {
	...
	**/
	public function delete()
    {
        $m_event = M('leads');
        if ($this->isPOST()) {
            $leads_id = I('post.leads_id/d');
            if ($m_event->where("leads_id= $leads_id")->delete()) {
                return;
            } else {
                $this->error('删除失败');
            }
	...
	**/
    ...
    public function listDialogs() {
        //1表示所有人  2表示下属
        $by = 'all';
        $all_or_below = ($by == 'all' || $by == 'message' || session('?admin')) ? 1 : 0;
        ...
        $role_arr = session('user_id');

        $where['user.role_id'] = array('in', $role_arr);
        $where['user.status'] = array('eq',1);
        $role_list = $d_role->where($where)->order('role_id')->select();
	...
	**/
	public function analytics(){
		...
		if(intval($_POST['role'])){
			$role_ids = array(intval($_POST['role']));
		}else{
			if(intval($_POST['department'])){
		...
		//时间段搜索
        if($_POST['begin_time'] && $_POST['next_time']){
            $between_time = $_POST['begin_time'].' - '.$_POST['next_time'];
        	$between_date = explode(' - ',trim($between_time));
			if($between_date[0]){
		...
		$department_val = $_POST['department'];
        $this->assign('department',$department_val);
        $this->assign('begin_time',$_POST['begin_time']);
        $this->assign('next_time',$_POST['next_time']);

		$this->daterange = daterange();
		$y_data = array();
        for($i=0;$i<count($reportList);$i++){
            $y_data[$i] = "'".$reportList[$i]['user']['user_name']."'".",";
            $conversion[$i] = $reportList[$i]['success_rate'];
        }
        $comma_separated = implode("", $y_data);
        $comma_separateds = implode(",", $conversion);

        $this->assign('comma_separated',$comma_separated);
        $this->assign('comma_separateds',$comma_separateds);
		$this->type_id = intval($_POST['type_id']);
