...
class FinanceAction extends Action{
	...
	**/
	public function _initialize(){
		$action = array(
			'permission'=>array(),
			'allow'=>array('listdialog', 'revert', 'adddialog', 'analytics', 'checkout','getmonthlyreceive','getyearreceivecomparison','getreceivablesmoney','getpayablesmoney','receivablesplan','advance_search','getcurrentstatus','revokecheck')
		);
	...
	**/
	public function index(){
		...
		$by = trim($_GET['by']) ? trim($_GET['by']) : 'all';
		$this->by = $by;
		...
		if ($_REQUEST["field"]) {
			...
			if ($field == "receivingorder.name" && $search != '') {
				$temp_where = array();
				$c_where['name'] = array('like','%'.$search.'%');
				$customer_ids = M('Customer')->where($c_where)->getField('customer_id', true);
				$temp_where['receivables.customer_id'] = array('in', $customer_ids);
				$temp_where['receivingorder.name'] = $where['receivingorder.name'];
				$temp_where['_logic'] = 'or';
				$where['_complex'] = $temp_where;
				unset($where['receivingorder.name']);
			}
			...
			if(trim($_REQUEST['field']) == 'owner_role_id'){
				if(!in_array(trim($search),$below_ids)){
					$where[$this->type . '.owner_role_id'] = array('in',$below_ids);
				}
			}
		}
		...
		switch ($this->type) {
			case 'receivables' :
				...
				if(!$_GET['field']){
					...
					foreach($_GET as $kd => $vd){
						...
						if(!in_array($kd,$no_field_array)){
							if(in_array($kd,array('create_time','update_time','pay_time'))){
								...
								if(!empty($vd['value']) || in_array($vd['condition'], array('is_empty', 'is_not_empty'))){
									$c_where['name'] = field($vd['value'], $vd['condition']);
									$customer_ids = M('Customer')->where($c_where)->getField('customer_id',true); 
									if($customer_ids){
								...
								if(!empty($vd['value']) || in_array($vd['condition'], array('is_empty', 'is_not_empty'))){
									$c_where['number'] = field($vd['value'], $vd['condition']);
					...
					$d_search = D('Search');
					$where = $d_search->roleWhere($this->type.'.owner_role_id', $where, $this->_permissionRes);
				}
				...
				foreach($list as $k=>$v){
					...
					if ($v['type'] == 2) {
						$list[$k]['type_name'] = '采购退货';
						$list[$k]['supplier_name'] = $m_supplier->where(array('supplier_id' => $v['customer_id']))->getField('name');

						$list[$k]['purchase_return_name'] = $m_sales->where(array('sales_id' => $v['contract_id']))->getField('sn_code');
					} else {
						$list[$k]['type_name'] = '销售';
					}
				}
			...
			case 'payables' :
				...
				foreach($list as $k=>$v){
					...
					switch ($v['type_id']) {
						case '-1':
							$list[$k]['type_name'] = '采购付款';
							$list[$k]['supplier_name'] = M('Supplier')->where('supplier_id = %d', $v['customer_id'])->getField('name');
							break;
						case '-2':
							$list[$k]['type_name'] = '销售退货付款';
							$multiple_info = array();
							$multiple_info = D('Customer')->getInfoByPurchaseId($v['contract_id']);
							$list[$k]['customer_name'] = $multiple_info['customer_info']['name'];
							$list[$k]['customer_id'] = $multiple_info['customer_info']['customer_id'];
							break;
						default:
							$list[$k]['type_name'] = M('FinanceType')->where('id = %d', $v['type_id'])->getField('name');
							break;
					}
				}
				$sum_money = number_format($sum_money,2);
			...
			case 'receivingorder' :
				...
				if(!$_GET['field']){
					...
					foreach($_GET as $kd => $vd){
						...
						if(!in_array($kd,$no_field_array)){
							...
							}elseif($kd =='customer_name' || $kd =='supplier_name'){
								if(!empty($vd['value']) || in_array($vd['condition'], array('is_empty', 'is_not_empty'))){
									if ($kd =='customer_name') {
										$c_where['name'] = field($vd['value'], $vd['condition']);
										$customer_ids = M('customer')->where($c_where)->getField('customer_id',true); 
									} else {
										$s_where['name'] = field($vd['value'], $vd['condition']);
										$customer_ids =$m_supplier->where($c_where)->getField('supplier_id',true); 
									}
									if($customer_ids){
										$where['customer_id'] = array('in',$customer_ids);
									}else{
				...
				foreach($list as $k=>$v){
					...
					//合同下应收款
					$receivables_id_arr = array();
					$receivables_id_arr = $m_receivables->where(array('contract_id'=>$v['contract_id']))->getField('receivables_id',true);
					$receivingorder_price_total = $m_receivingorder->where(array('receivables_id'=>array('in',$receivables_id_arr),'status'=>1))->sum('money');
					//剩余金额
					...
					if ($v['receivables_type'] == 2) {
						$sales_info = $m_sales->where(array('sales_id' => $v['contract_id']))->field('sn_code, sales_price')->find();
						$list[$k]['supplier_name'] = $m_supplier->where(array('supplier_id' => $v['customer_id']))->getField('name');
						$list[$k]['purchase_return_number'] = $sales_info['sn_code'];
						$list[$k]['purchase_return_price'] = $sales_info['sales_price'];
					}
				}
				$sum_money = number_format($sum_money,2);
				$money_arr = array('sum_money'=>$sum_money);
				...
				if(!$_GET['field']){
					...
					foreach($_GET as $kd => $vd){
						...
						if(!in_array($kd,$no_field_array)){
							if(in_array($kd,array('create_time','update_time','pay_time'))){
							...
							}elseif(in_array($kd,array('status'))){
								if(!empty($vd)){
									$where[$this->type .'.'.$kd] = $vd['value'];
									$fields_search[$kd]['field'] = $kd;
									$fields_search[$kd]['value'] = $vd['value'];
								...
								$fields_search[$kd]['condition'] = $vd['condition'];
								$fields_search[$kd]['value'] = $vd['value'];
							}elseif(in_array($kd, array('owner_role_id','creator_role_id'))){
								if(!empty($vd)){
									$where['paymentorder.'.$kd] = field($vd['value'], $vd['condition']);
								}
								$fields_search[$kd]['field'] = $kd;
								$fields_search[$kd]['condition'] = $vd['condition'];
								$fields_search[$kd]['value'] = $vd['value'];
	...
	public function add(){
		switch ($this->type) {
			case 'receivables' :
				...
				$m_receivables = M('Receivables');
				if($this->isPost()){
					if ($res = $m_receivables->create()) {
						if(empty($_POST['customer_id'])){
							$this->error(L('PLEASE_SELECT_CUSTOMERS'));
						} 
						$m_receivables->type = $_POST['type'] ?: 1;
						...
						if($id = $m_receivables->add()){
							...
							if(!empty($id)){
								...
								}else if($_POST['refer_url'] == 3){
									alert('success', '添加成功', $_POST['refer_url2']);
								}else{
				...
				if($this->isPost()){
					...
					if ($m_payables->create()) {
						...
						if($id = $m_payables->add()){
							...
							$url = $_POST['refer_url'] ?: U('finance/index', 't=payables');
							alert('success',L('ADD SUCCESS',array('')), $url);
						}else{
							$this->error(L('ADDING FAILS CONTACT THE ADMINISTRATOR',array('')));
						}
					} else {
			...
			case 'receivables' :
				...
				if($this->isPost()){
					...
					if ($info['type'] == 1) {
						...
						$info['purchase_return_name'] = M('Sales')->where(array('sales_id' => $info['contract_id'], 'type' => 1))->getField('sn_code');
						$info['supplier_name'] = M('Supplier')->where(array('supplier_id' => $info['customer_id']))->getField('name');
					}
			...
			case 'payables' :
				...
				if($this->isPost()){
					...
					switch ($info['type_id']) {
						case '-1':
							$info['type_name'] = '采购付款';
							$info['supplier_name'] = M('Supplier')->where('supplier_id = %d', $info['customer_id'])->getField('name');
							$info['purchase_number'] = M('Purchase')->where('purchase_id = %d', $info['contract_id'])->getField('number');
							break;
						case '-2':
							$info['type_name'] = '销售退货付款';
							$multiple_info = array();
							$multiple_info = D('Customer')->getInfoByPurchaseId($info['contract_id']);
							$info += $multiple_info;
							break;
						default:
							$info['type_name'] = M('FinanceType')->where('id = %d', $info['type_id'])->getField('name');
							break;
					}
	...
	public function view(){
		...
		switch ($this->type) {
			case 'receivables' :
				...
				if ($info['type'] == 2) {
					// 采退单名称
					$info['purchase_return_name'] = M('Sales')->where(array('sales_id' => $info['contract_id']))->getField('sn_code');
				}
			...
			case 'receivingorder' :
				...
				$info['examine']['img'] = $info['examine']['thumb_path'] = headPathHandle($info['examine']['thumb_path']);
				if(empty($info)){
					$this->error(L('RECORD NOT EXIST'));
				}elseif(!in_array($info['owner_role_id'], $this->_permissionRes)){
					alert('error',L('DO NOT HAVE PRIVILEGES'),$_SERVER['HTTP_REFERER']);
				}
				$temp_val = $d_user->get_full_name(array($info['owner_role_id']));
				$info['owner'] = $temp_val[$info['owner_role_id']];
				if (!$info['creator_role_id']) {
					$info['creator_role_id'] = $info['owner_role_id'];
					$info['creator_name'] = $info['owner']['full_name'];
				}
				// 判断是否有审核权限
				$info['do_examine'] = D('Finance')->do_examine($info['status']);
			...
			case 'paymentorder' :
				...
				$info['owner'] = $temp_val[$info['owner_role_id']];
				//银行账户
				$info['receipt_bank'] = M('bank_account')->where(array('account_id'=>$info['bank_account_id']))->getField('open_bank');
				$this->assign('info',$info);
				$this->alert = parseAlert();
	...
	public function listdialog(){
		...
		if ($_REQUEST["field"]) {
			...
			if(trim($_REQUEST["field"]) == "customer_id"){
				...
				$customer_ids = M('Customer')->where($c_where)->getField('customer_id',true);
				$where[$field] = array('in',$customer_ids);
			}else if (trim($_REQUEST["field"]) == "contract_number"){
				$where['contract.number'] = array('like','%'.$search.'%');
			}else{
				switch ($_REQUEST['condition']) {
					case "is" : $where[$field] = array('eq',$search);break;
					case "isnot" :  $where[$field] = array('neq',$search);break;
		...
		switch ($this->type) {
			case 'receivables' :
				...
				foreach ($list as $key => $val) {
					if ($val['type'] == 2) {
						$list[$key]['supplier_name'] = $m_supplier->where(array('supplier_id' => $val['customer_id']))->getField('name');
					}
				}
			...
			case 'payables' :
				...
				foreach ($payablesList as $k => $v) {
					//付款类型
					switch ($v['type_id']) {
						case '-1':
							$payablesList[$k]['type_name'] = '采购付款';
							$payablesList[$k]['supplier_name'] = M('Supplier')->where('supplier_id = %d', $v['customer_id'])->getField('name');
							break;
						case '-2':
							$payablesList[$k]['type_name'] = '销售退货付款';
							$multiple_info = array();
							$multiple_info = D('Customer')->getInfoByPurchaseId($v['contract_id']);
							$payablesList[$k]['customer_name'] = $multiple_info['customer_info']['name'];
							$payablesList[$k]['customer_id'] = $multiple_info['customer_info']['customer_id'];
							break;
						default:
							$payablesList[$k]['type_name'] = M('FinanceType')->where('id = %d', $v['type_id'])->getField('name');
							break;
					}
				}
	//p($payablesList);
		...
		switch ($this->type) {
			case 'receivables' :
				if ($_GET['type'] == 2) {
					...
					$this->type = 2;
					$default = M('Sales s')->where(array('s.sales_id' => $id))
						->join('LEFT JOIN '. C('DB_PREFIX') .'purchase p ON s.customer_id=p.purchase_id')
						->join('LEFT JOIN '. C('DB_PREFIX') .'supplier su ON su.supplier_id=p.type_id AND p.type=1')
						->field('s.sn_code AS order_number, su.name AS supplier_name, supplier_id')
						->find();
					$default['id'] = $id;
					$default['url_type'] = 3;
					$default['url'] = U('purchase/return_goods_view') .'&id='. $id .'#tab3';
				...
				break;
			case 'payables' :
				if (!$id = (int) $_GET['purchase_id']) {
					echo '<div class="alert alert-error">参数错误！</div>';die;
				}
				$this->id = $id;
				$this->refer_url = U('purchase/view') . '&id='. $id .'#tab3';
				//应付款
				$this->object_id = M('Purchase')->where(array('purchase_id' => $id))->getField('type_id');
				if ((int) $_GET['type'] == 2) {
					$this->type = array('val' => -2, 'name' => '销售退货付款', 'object' => '客户');
					$this->object_name = M("Sales s")->where('s.sales_id='. $this->object_id)
						->join('LEFT JOIN '. C('DB_PREFIX') .'customer c ON c.customer_id=s.customer_id')
						->getField('c.name');
				} else {
					$this->type = array('val' => -1, 'name' => '采购付款', 'object' => '供应商');
					$this->object_name = M('Supplier')->where(array('supplier_id' => $this->object_id))->getField('name');
				}
				$payables_max_id = (int) M('Payables')->max('payables_id') + 1;
				$payables_custom = M('config') -> where('name="payables_custom"')->getField('value');
				$this->number = $payables_custom . date('Ymd') .'-'. str_pad($payables_max_id, 4, 0, STR_PAD_LEFT);
	...
	**/
	...
	public function excelExport($financeList=false,$type){
		...
		$objProps->setTitle("pdcrm Konwledge");    
		$objProps->setSubject("pdcrm Konwledge Data");    
		$objProps->setDescription("pdcrm Konwledge Data");    
		$objProps->setKeywords("pdcrm Konwledge");    
        ...
        header("Content-Disposition:attachment;filename=pdcrm_receivingorder_".date('Y-m-d',mktime())."_".$current_page.".xls");
