...
class TaskAction extends Action{
	...
	public function add(){
		if($this->isPost()){
			...
			if ($_REQUEST['customer_id']) {
				...
				$customer['name'] = M('customer')->where('customer_id = %d', $_REQUEST['customer_id'])->getField('name');
	...
	**/
	public function index(){
		...
		foreach($type_list as $k=>$v){
			...
			foreach ($task_list as $key=>$value) {
				...
				$task_list[$key]['about_roles'] = $d_user->get_full_name($about_roles_id);
	...
	public function widget()
	{
		...
		$t_where['role_id'] = session('role_id');
		$t_where['is_deleted'] = array('neq', 1);
		$type = $m_task_type->where($t_where)->field('id,name')->select();
		$type_ids = array();
		$type_name = array();
		...
		$order = "update_date desc,order_id asc,task_id asc";
		$where['type_id'] = array('in', implode(',', $type_ids));
		$where['is_deleted'] = array('neq', 1);
		$where['status'] = array('neq', '完成');
		$count = $m_task->where($where)->count();
		$task = $m_task->field('task_id,subject,status,type_id,priority')->where($where)->order($order)->page($p.','.$page_size)->select();
		...
		if ($p == 1) {
			...
			$page .= ' <a class="prev_task_'.$widget_id.'" href="javascript:void(0);" style="padding: 0 10px; border: 1px solid #ccc;"><i class="glyphicon glyphicon-chevron-left"></i></a> ';
		}
		if ($p == ceil($count / $page_size)) {
			...
			$page .= ' <a class="next_task_'.$widget_id.'" href="javascript:void(0);" style="padding: 0 10px; border: 1px solid #ccc;"><i class="glyphicon glyphicon-chevron-right"></i></a> ';
		}
		$page .= '</div>';
		$page .= "
			<script>
				$('.prev_task_".$widget_id."').on('click', function(){
					task_page_".$widget_id." -= 1;
					taskWidgetGetData_".$widget_id."($(this));
				});
				$('.next_task_".$widget_id."').on('click', function(){
					task_page_".$widget_id." += 1;
					taskWidgetGetData_".$widget_id."($(this));
				});
			</script>
		";
		$priority = array('普通' => '46BE8A', '高' => 'F96868', '低' => 'F2A654', '无' => 'ccc');
		foreach ($task as $key => $val) {
			$done = 0;
			$sub = $m_task_sub->where('task_id=%d', $val['task_id'])->field('is_done')->select();
			foreach ($sub as $k => $v) {
				if ($v['is_done'] == 1) $done++; 
			}
			$task[$key]['done'] = floor($done / count($sub) * 100);
			$task[$key]['task_type'] = $type_name[$val['type_id']];
			$task[$key]['priority'] = $val['priority'] ? $val['priority'] : '无';
			$task[$key]['p_color'] = $priority[$task[$key]['priority']];
		}
		$this->ajaxReturn(array('task' => $task, 'page' => $page), '', 1);
	}
	...
	public function view() {
		...
		if (0 == $task_id) {
			...
			if(!session('?admin') && $task['creator_role_id'] != session('role_id') && !in_array(session('role_id'),array_filter(explode(',',$task['about_roles']))) && !in_array(session('role_id'),array_filter(explode(',',$task['owner_role_id'])))){
				...
				$task['owner'] = $d_user->get_full_name(explode(',', $task['owner_role_id']));
				$task['creator'] = getUserByRoleId($task['creator_role_id']);
				$task['about_roles_list'] = $d_user->get_full_name(explode(',', $task['about_roles']));
				...
				foreach ($task['file_list'] as $key => $value) {
					...
					if ($value['oss'] == 1) {
						$task['file_list'][$key]['file_path'] = $d_file::FILE_URL . '/' . $value['file_path'];
					}
				}
				...
				foreach($group_date as $key=>$val){
					...
					foreach($action_list as $k=>$v){
						$create_role_info = $d_user->get_full_name(array($v['role_id']));
						$action_list[$k]['create_role_info'] = $create_role_info[$v['role_id']];
						$role_list = array();
						if($v['about_role_id']){
	...
	**/
	public function excelExport($taskList=false){
		...
		$objProps->setTitle("pdcrm Task Data");    
		$objProps->setSubject("pdcrm Task Data");    
		$objProps->setDescription("pdcrm Task Data");    
		$objProps->setKeywords("pdcrm Task Data");    
        ...
        header("Content-Disposition:attachment;filename=pdcrm_task_".date('Y-m-d',mktime())."_".$current_page.".xls");
