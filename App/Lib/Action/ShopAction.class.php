	...
	public function lostpw() {               
		if($this->isAjax()){
			if ($_POST['telephone'] || $_POST['email']){
				...
				//$verify_code = md5(md5($time));
				//import('@.ORG.Mail');
				//$url = "http://www.xxx.com/index.php?m=server&a=resetpw&user_id=".$info['user_id']."&verify_code=".$verify_code;
				//$content ='尊敬的' . $_POST['telephone'] . '：<br/><br/>请点击下面的链接完成找回密码：<br/><br/><a href="' . $url .'">点击此处，找回密码！</a><br/><br/>--PDCRM(这是一封自动产生的email，请勿回复。)';
				//if (SendMail($info['email'],'找回密码链接',$content,'PDCRM管理员')){
				//$url = "http://www.xxx.com/index.php?m=server&a=resetpw&user_id=".$info['user_id']."&verify_code=".$verify_code;
				//$content ='尊敬的' . $_POST['telephone'] . '：<br/><br/>请点击下面的链接完成找回密码：<br/><br/><a href="' . $url .'">点击此处，找回密码！</a><br/><br/>--PDCRM(这是一封自动产生的email，请勿回复。)';
				//if (SendMail($info['email'],'找回密码链接',$content,'PDCRM管理员')){
					//notice('邮件发送成功，请在24小时之内到邮箱查看，请留意垃圾邮件！' , U('server/login'));
				//}
			} else {
