...
*
**/
class EventmAction extends Action{
	/**
	*用于判断权限
	*@permission 无限制
	*@allow 登录用户可访问
	...
	**/
	public function _initialize(){
		$action = array(
			'permission'=>array(''),
			'allow'=>array('close','open','event_dialog','getcurrentstatus','editable','indexdata','add','view','delete','edit', 'view_ajax')
		);
	...
	**/
	...
	/*	if ($this->isPost()){
		    $suject=trim($_POST['subject']);
		    $start_time=$_POST['start_date']?strtotime($_POST['start_date']):strtotime(date('Y-m-d',time()));
		    $end_time=$_POST['end_date']?strtotime($_POST['end_date']):strtotime(date('Y-m-d'),time());
		...
		if($this->isPost()){
			...
			// $this->alert = parseAlert();
            $this->start_date = $_GET['start_date'] ? date('Y-m-d H:i',strtotime($_GET['start_date'])) : date('Y-m-d H:i',time());
			$this->end_date = $_GET['start_date'] ? date('Y-m-d H:i',(strtotime($_GET['start_date'])+86399)) : date('Y-m-d H:i',strtotime(date('Y-m-d',(time())))+86399);
			if ($_GET['module'] && $_GET['module_id']) {
				$module = array('customer' => '客户', 'business' => '商机');
				$module_name = M($_GET['module'])->where($_GET['module'] . '_id=%d', $_GET['module_id'])->getField('name');
				$this->module = array('name' => $module_name, 'id' => $_GET['module_id'], 'module_name' => $_GET['module'], 'module' => $module[$_GET['module']]);
			}
	...
	**/
	public function edit(){
		...
		if ($this->isPost()) {
			...
			if($m_event->where("event_id=$event_id")->save()){
				// actionLog($event_id);
	...
	**/
	public function delete(){
		...
		if($this->isPost()){
			...
			if ($m_event->where("event_id= $event_id")->delete()){
			    $this->redirect('../Eventm/index');
            }else{
			    $this->error('删除失败');
            }
	...
	**/
	public function index(){
        ...
        $nextTimestamp=$timestamp+86399;

        //查找日程数据
        $event=$m_event->field("FROM_UNIXTIME(start_date,'%Y-%m-%d %H:%i') as start_date,
        FROM_UNIXTIME(end_date,'%Y-%m-%d %H:%i') as end_date,event_id,description")
            ->where("(start_date<=$timestamp and end_date>=$nextTimestamp) 
            or (start_date>=$timestamp and end_date<=$nextTimestamp)
            or (start_date<=$timestamp and end_date>=$timestamp and end_date<=$nextTimestamp)
           or (start_date>=$timestamp and start_date<=$nextTimestamp and end_date>=$nextTimestamp)")->select();
		/*$event=$m_event->field('start_date,end_date,event_id,description')->where("start_date<=$newtime and $newtime<=end_date")->select();*/
     ...
     * 日程加点
     */
    public function evePoint(){
        $m_event=M('event');
        $event=$m_event->field("start_date,end_date,event_id")->select();
     ...
     * 点击查看日程信息
     */
    public function eveInfo(){

        $day=$_POST['day'];
        $nextDay=intval($day)+1;
        $year=$_POST['year'];
        $date=$year."/".$day;
        $nextDate=$year."/".$nextDay;
	...
	**/
	public function view(){
		...
		switch($event_info['module']){
			...
			case 'customer' : 
				$relevant_name = '客户';
				$customer_info = $m_customer->where(array('customer_id'=>$event_info['module_id']))->field('nextstep_time,name')->find();
				$name = $customer_info['name'];
				if ($customer_info['nextstep_time'] == $event_info['start_date']) {
					$not_edit = 1;
				}
				break;
			case 'contacts' :
			...
			case 'remind' :
				...
				// 访客计划
				if ($visitor_plan = D('VisitorPlan')->getView(array('event_id' => $event_id))) {
					$remind_info['visitor_plan'] = $visitor_plan;
					$remind_info['visitor_plan_type_list'] = D('VisitorPlan')->type_list;
				}
				$field_name = 'name';
	...
	**/
	public function excelExport($eventList=false){
		...
		$objProps->setTitle("pdcrm Event Data");    
		$objProps->setSubject("pdcrm Event Data");    
		$objProps->setDescription("pdcrm Event Data");    
		$objProps->setKeywords("pdcrm Event Data");    
        ...
        header("Content-Disposition:attachment;filename=pdcrm_event_".date('Y-m-d',mktime())."_".$current_page.".xls");
		...
		foreach($list as $k=>$v){
			...
			switch ($v['module']) {
				...
				case 'remind' : 
					...
					break;
				case 'customer' :
					$customer_info = $m_customer->where(array('customer_id'=>$v['module_id']))->field('nextstep_time,name')->find();
					if ($customer_info['nextstep_time'] == $v['start_date']) {
						$subject_info = '【 访客提醒 】' . $customer_info['name'];
					} else {
						$subject_info = '【 '.$customer_info['name'].' 】';
					}
					break;
				case 'receivables' :
	...
	**/
	...
	public function view_ajax()
	{
		...
		if($customer_id){
			$business_ids = $m_business->where('customer_id =%d',$customer_id)->getField('business_id',true);
			if($business_ids){
				$where = array();
		...
		$p = (int) $_REQUEST['p'] ?: '1';
		$event_list = $m_event ->where($where)->page($p.', 10')->select();
		...
		$this->ajaxReturn(array('data' => $event_list, 'msg' => '', 'status' => (int) $event_list));
	}
}
