...
class IndexmAction extends Action
{
    ...
    public function index()
    {
        ...
        $banner = M('banner');
        $factor['status'] = array('neq',2);
        $list = $banner->where($factor)->order(array('order'=>'desc'))->select();
        $this->assign("list",$list);
        $this->alert = parseAlert();

        $action = get_action_name();
        $moule = MODULE_NAME;
        $this->assign("md_name",$moule);
        $this->display($action);
    }

    //简报
    public function brielog()
    {
        $user=M('User');
        $dashboard=$user->where('user_id=%d',session('user_id'))->getField('dashboard');
        $widget=unserialize($dashboard);
     ...
     * @return
     */
    public function chatReg()
    {
        $user_id = $_POST['user_id'];
        $user = M('user');
        $userRow = $user->where('user_id='.$user_id)->field('name')->find();
        //为该用户设置极光账号密码
        if ($userRow) {
            $where['user_id'] = $user_id;
            $data['chatname'] = $userRow['name'];
            $data['chatpwd'] = '123456';
        ...
        $userRes = $user->field('user_id,chatname,chatpwd')->where('user_id='.$user_id)->find();

        $jim = new JMessage();
        return json_encode($jim->openRegister(array(
            'username' =>$userRes['chatname'] ,
            'password' => $userRes['chatpwd'])));  //注册用户
        //var_dump($jim->getUserDetails('orange'));

        //print_r($jim->addfriends('orange',array("wakakai")));     //加好友

        //发送消息
        //var_dump($jim->sendMsg('shinjae97', '天气不错哦', 'single', array(), 'orange'));
