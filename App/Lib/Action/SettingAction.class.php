...
class SettingAction extends Action{
	public function _initialize(){
		$action = array(
			...
			'allow'=>array('getbusinessstatuslist','boxfield','mapdialog','customeshow','lockscreen','getreplybystatus','replylist','logreplyadd','logreplyedit','logreplydel','workrule','workruleedit','workruledel','smtp','action_record')
		);
		B('Authenticate',$action);
	}

	//自定义模块
	public $model_array = array('leads','customer','contacts','product','business','contract','examine','market','supplier');
	...
	**/
	public function fields(){
		...
		if ($model == 'examine') {
			...
			$examine_status = M('ExamineStatus')->where(array('status'=>$status))->find();
			if (!$examine_status) {
				alert('error','参数错误！',$_SERVER['HTTP_REFERER']);
			}
			if ($status < 7) {
	...
	**/
	public function fieldAdd(){
		...
		if ($model == 'examine') {
			//审批自定义
			if ($this->isPost() && !$status) {
				alert('error','参数错误！',$_SERVER['HTTP_REFERER']);
			}
			if ($status < 7) {
				echo '<div class="alert alert-error">参数错误！</div>';die();
			}
			if (!M('ExamineStatus')->where(array('status'=>$status))->find()) {
				echo '<div class="alert alert-error">参数错误！</div>';die();
			}
		}
		...
		if($this->isPost()){
			...
			$data['status']       = $status;
			if($field->where(array('field'=>$field_str,'model'=>array(array('eq',$data['model']),array('eq',''),'OR'),'status'=>$field_info['stauts']))->find()){
				alert('error',L('THE FIELD NAME ALREADY EXISTS'),$_SERVER['HTTP_REFERER']);
			}
			if($field->where(array('name'=>$data['name'],'model'=>array(array('eq',$data['model']),array('eq',''),'OR'),'status'=>$field_info['stauts']))->find()){
	...
	**/
	public function sendSms(){
		if($this->isPost() && $_POST['is_get'] != 'true'){
			...
			$this->smsContent = $_POST['smsContent'];
	...
	public function smsRecord(){	
		...
		$list = $m_sms_record->where($where)->order($order)->page($p.','.$listrows)->select();
		$m_user = M('User');
		foreach($list as $k=>$v){
			//查询发送人
			$list[$k]['role_name'] = $m_user->where('role_id=%d',$v['role_id'])->getField('full_name');
			//截取手机号
	...
	**/
	...
	public function examine(){
		...
		foreach ($examine_status_list as $k=>$v) {
			...
			$img_repeat = '';
			switch ($v['status']) {
				case '1' : $description = '适用于公司日常审批';
							$img_repeat = '-50px';
							break;
				case '2' : $description = '适用于公司请假审批'; 
							$img_repeat = '0px';
							break;
				case '3' : $description = '适用于公司日常报销'; 
							$img_repeat = '-100px';
							break;
				case '4' : $description = '适用于公司差旅报销';
							$img_repeat = '-150px';
							 break;
				case '5' : $description = '适用于公司出差申请'; 
							$img_repeat = '-200px';
							break;
				case '6' : $description = '适用于公司借款申请';
							$img_repeat = '-250px';
							 break;
			}
			if ($v['status'] > 6) {
				if ($v['status'] > 18) {
					$img_repeat = (-($v['status']-13)*50).'px';
				} else {
					$img_repeat = (-($v['status']-1)*50).'px';
				}
			}
			$examine_status_list[$k]['description'] = $description ? : $v['description'];
			$examine_status_list[$k]['img_repeat'] = $img_repeat;
		}
    ...
    public function edit()
    {
        ...
        if($list){
            ...
            alert('error', '信息不存在！', $_SERVER['HTTP_REFERER']);
        }
        $this->alert=parseAlert();
        $this->display();
    }
    ...
    public function save()
    {
        ...
        if($_POST["id"]){
            $where['id'] = $_POST["id"];
            $img = $model->where($where)->field('img_path')->find();
            unlink($img['img_path']);
        ...
        if($info){
            $model->commit();
            alert('success','banner添加成功',U('setting/banner'));
        }else{
            $model->rollback();
            alert('error','banner添加失败',U('setting/banner'));
        }
    ...
    public function doUploadPic()//上传模块
    {
        ...
        if(!$upload->upload()) {// 上传错误提示错误信息
            ...
            $info = $upload->getUploadFileInfo();
            $data = $info[0]['savepath'] . $info[0]['savename'];
            $file_a=$data;
