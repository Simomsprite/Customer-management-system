	...
	public function index(){
		...
		if ($_REQUEST["field"]) {
			...
			if (isset($where['title'])) {
				$map['title'] = array('LIKE', '%' . $where['title'][1] . '%');
				$map['content'] = array('LIKE', '%' . $where['title'][1] . '%');
				$map['_logic'] = 'OR';
				$where['_complex'] = $map;
				unset($where['title']);
			}
...
//p($where);
	...
	**/
	public function excelExport($knowledgeList=false){
        ...
        header("Content-Disposition:attachment;filename=pdcrm_knowledge_".date('Y-m-d',mktime())."_".$current_page.".xls");
