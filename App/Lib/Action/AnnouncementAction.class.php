...
class AnnouncementAction extends Action{
	...
	**/
	public function _initialize(){
		$action = array(
			'permission'=>array(),
			'allow'=>array('read_list', 'getannouncement')
		);
	...
	**/
	public function index(){
			...
			foreach($_GET as $kd => $vd){
                if ($kd != 'act' && $kd != 'content' && $kd != 'p' && $kd != 'search') {
					if(in_array($kd,array('role_id'))){
						...
						if(is_array($vd)) {
							if(!empty($vd['value']) || in_array($vd['condition'], array('is_not_empty', 'is_empty'))){
								$where[$kd] = field($vd['value'], $vd['condition']);
								$fields_search[$kd]['field'] = $kd;
								$fields_search[$kd]['condition'] = $vd['condition'];
								$fields_search[$kd]['value'] = $vd['value'];
		...
		$list = $m_announcement->where($where)->order($order)->Page($p.','.$listrows)->select();
	...
	**/
	public function view(){
		...
		if(!in_array('('.session('department_id').')', explode(',',$announcement_info['department'])) && !session('?admin')){
	...
	**/
	public function read_list(){
		...
		if(!session('?admin') && !in_array('('.session('department_id').')', explode(',',$announcement_info['department']))){
			alert('error','您没有此权利！',$_SERVER['HTTP_REFERER']);
		}
	...
	**/
	...
	public function getAnnouncement(){
		...
		if (!getPerByAction('announcement','index')) {
		...
		$announcement = $m_announcement->field('announcement_id,order_id,role_id,title,create_time,update_time,color,department,status,isshow')->where('')->order('order_id')->page($p.','.$page_size)->select();
		
		foreach ($announcement as $key => $val) {
			$announcement[$key]['is_read'] = M('announcement_data')->where(array('announcement_id' => $val['announcement_id'], 'role_id' => session('role_id')))->count();
		}
		...
		$page .= "
			<script>
				$('.prev_widget_".$widget_id."').on('click', function(){
					...
					announcementWidgetGetData_".$widget_id."($(this));
				});
		...
		if(!empty($announcement)){
			$this->ajaxReturn(array('data' => $announcement, 'page' => $page),'success',1);
		}else{
			$this->ajaxReturn('','---暂无数据---',0);
		}
	}
