	...
	**/
	public function index(){
		...
		foreach($sign_list as $k=>$v){
			...
			$sign_list[$k]['user_info']['thumb_path'] = headPathHandle($sign_list[$k]['user_info']['thumb_path']);
			$sign_list[$k]['user_info']['img'] = headPathHandle($sign_list[$k]['user_info']['img']);
		...
		$role_list = D('RoleView')->where('role.role_id in (%s)', implode(',', $below_ids))->select();
		foreach ($role_list as $key => $val) {
			$role_list[$key]['img'] = headPathHandle($val['img']);
	...
	public function view(){
		...
		if($role_id && !in_array($role_id,$below_ids)){
			...
			foreach($sign_list as $k=>$v){
				...
				//获取头像
				$user_img = '"' . headPathHandle($user_info['thumb_path']) . '"';
				...
				foreach($sign_img_list as $k=>$v){
					$v['path'] = headPathHandle($v['path'], 1);
