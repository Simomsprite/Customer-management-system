<?php 
    ...
    /**
     * 搜索字段list信息
     * @param form_type类型决定了搜索的表单类型 text【普通文本】b_box【商机进度】role【下拉选择系统员工】data【选择日期】
     * @author lee
     */
    public function search_field_list()
    {
    	// 自定义字段
    	$field_list = getMainFields('business');
    	foreach ($field_list as $key => $val) {
			if (in_array($val['field'], array('status_id', 'customer_id', 'contacts_id'))) {
    	...
    	$search_field_list = array(
			...
			array('field' => 'contacts_name', 'name' => '联系人名称', 'form_type' => 'text'),
			array('field' => 'contacts_telephone', 'name' => '联系人电话', 'form_type' => 'text'),
