...
class IndexMobile extends Action{
	...
	public function comment2(){
		$role_id = session('role_id');
		$page = isset($_POST['p']) ? intval($_POST['p']) : 1 ;
		$comment_sql = M()->query("(select * from `pdcrm_comment` where module='log' and to_role_id = ".$role_id." group by module_id order by create_time desc) order by create_time desc limit 10");
		$comment_count_array = mysql_query("select count(*) from `pdcrm_comment` where module='log' and to_role_id = ".$role_id." group by module_id");
