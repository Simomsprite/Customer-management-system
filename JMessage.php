...
class JMessage
{
     ...
     */
    public function sendMsg($username, $content, $target_type, $ext, $from_user )
    {
        $url = $this->url . "/v1/messages";
        $options = array(
            "version" => 1, //版本号
            "target_type" =>"single",  //发送目标类型 single - 个人，group - 群组
            "target_id" => $username, //目标id single填username, group 填gid
            "from_type" => "user", //发消息着身份 当前只限admin
            "from_id" => $from_user,    //发送者的username
            "msg_type" => "text",   //发消息类型 当前只限text
            "msg_body" => array(    //消息体
                "extras" => array(""=>""), //选填的json对象 开发者可以自定义extras里面的key value
                "text" => $content    //消息内容
            )
        );
        $result = $this->postCurl($url, $options);
    ...
    public function sendImg($username, $media_id,$media_crc32,$width,$height, $fsize,$target_type, $ext, $from_user )
    {
        $url = $this->url . "/v1/messages";
        $options = array(
            "version" => 1, //版本号
    ...
    public function addfriends($user, array $friends) {
        $uri = $this->url.'/v1/users/'. $user . '/friends';
        $body = $friends;
        $response = $this->postCurl($uri, $body);
        return $response;
    }
